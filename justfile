...
graph:
    $ErrorActionPreference = 'Stop'; $e = [char]27; function Write-Step { param($msg) Write-Host "$e[36m[GRAPH]$e[0m $msg" }; function Write-Success { param($msg) Write-Host "  $e[32m[OK]$e[0m $msg" }; function Write-Fail { param($msg) Write-Host "  $e[31m[FAIL]$e[0m $msg" }; function Write-Progress { param($msg) Write-Host "  $e[90m...$e[0m $msg" }; $startTime = Get-Date; Write-Host "`n$e[1;36m=== Generating Knowledge Graph ===$e[0m`n"; Write-Step "Checking dependencies..."; try { $pyVersion = pixi run python --version 2>&1; if ($LASTEXITCODE -ne 0) { throw "Python not available" }; Write-Success "Python: $pyVersion" } catch { Write-Fail "Python not available: $_"; exit 1 }; Write-Step "Verifying script exists..."; $scriptPath = "scripts\knowledge-graph\visualize_architecture.py"; if (-not (Test-Path $scriptPath)) { Write-Fail "Script not found: $scriptPath"; exit 1 }; Write-Success "Script found"; Write-Step "Generating knowledge graph (this may take 30-60 seconds)..."; Write-Progress "Parsing codebase modules and building graph structure"; $phase = "init"; $nodeCount = 0; $edgeCount = 0; try { pixi run python $scriptPath 2>&1 | ForEach-Object { $line = $_; if ($line -match "Parsed (\d+) files") { $phase = "parsed"; Write-Progress "Parsed $($matches[1]) source files" } elseif ($line -match "Building knowledge graph") { $phase = "building"; Write-Progress "Building knowledge graph structure" } elseif ($line -match "Graph: (\d+) nodes, (\d+) edges") { $phase = "graph"; $nodeCount = $matches[1]; $edgeCount = $matches[2]; Write-Progress "Graph built: $nodeCount nodes, $edgeCount edges" } elseif ($line -match "Calculating.*layout") { $phase = "layout"; Write-Progress "Calculating hierarchical layout" } elseif ($line -match "Generating.*HTML") { $phase = "html"; Write-Progress "Generating interactive HTML visualization" } elseif ($line -match "GraphML saved") { Write-Progress "GraphML exported to scripts/knowledge-graph/" } elseif ($line -match "ERROR|Error|error") { Write-Host "  $e[31m$line$e[0m" } }; if ($LASTEXITCODE -ne 0) { throw "Graph generation failed with exit code $LASTEXITCODE" }; Write-Success "Knowledge graph generated" } catch { Write-Fail "Generation failed: $_"; exit 1 }; Write-Step "Validating outputs..."; $htmlPath = "IntellicrackKnowledgeGraph.html"; $graphmlPath = "scripts\knowledge-graph\IntellicrackKnowledgeGraph.graphml"; if (-not (Test-Path $htmlPath)) { Write-Fail "HTML output not found: $htmlPath"; exit 1 }; $htmlSize = [math]::Round((Get-Item $htmlPath).Length / 1MB, 2); Write-Success "HTML: $htmlPath ($htmlSize MB)"; if (Test-Path $graphmlPath) { $graphmlSize = [math]::Round((Get-Item $graphmlPath).Length / 1MB, 2); Write-Success "GraphML: $graphmlPath ($graphmlSize MB)" }; $elapsed = ((Get-Date) - $startTime).TotalSeconds; Write-Host "`n$e[1;32m=== Knowledge Graph Complete ===$e[0m"; Write-Host "$e[90mNodes: $nodeCount | Edges: $edgeCount | Time: $("{0:N1}" -f $elapsed)s$e[0m"; Write-Host "View: $e[36mIntellicrackKnowledgeGraph.html$e[0m`n"
