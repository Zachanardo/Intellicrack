...
build-rust:
    @$ErrorActionPreference = 'Stop'; $e = [char]27; function Write-Step { param($msg) Write-Host "$e[36m[BUILD]$e[0m $msg" }; function Write-Success { param($msg) Write-Host "  $e[32m[OK]$e[0m $msg" }; function Write-Fail { param($msg) Write-Host "  $e[31m[FAIL]$e[0m $msg" }; $startTime = Get-Date; Write-Host "`n$e[1;36m=== Building Rust Launcher (Release) ===$e[0m`n"; Write-Step "Configuring MSVC environment..."; $vsPath = & "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe" -latest -property installationPath 2>$null; if ($vsPath) { $msvcPath = Get-ChildItem "$vsPath\VC\Tools\MSVC" -Directory | Sort-Object Name -Descending | Select-Object -First 1; if ($msvcPath) { $linkDir = "$($msvcPath.FullName)\bin\Hostx64\x64"; $libDir = "$($msvcPath.FullName)\lib\x64"; $incDir = "$($msvcPath.FullName)\include"; if (Test-Path "$linkDir\link.exe") { $env:PATH = "$linkDir;$env:PATH"; $sdkVer = Get-ChildItem "${env:ProgramFiles(x86)}\Windows Kits\10\Lib" -Directory | Sort-Object Name -Descending | Select-Object -First 1; if ($sdkVer) { $sdkBase = "${env:ProgramFiles(x86)}\Windows Kits\10"; $env:LIB = "$libDir;$($sdkVer.FullName)\ucrt\x64;$($sdkVer.FullName)\um\x64"; $env:INCLUDE = "$incDir;$sdkBase\Include\$($sdkVer.Name)\ucrt;$sdkBase\Include\$($sdkVer.Name)\um;$sdkBase\Include\$($sdkVer.Name)\shared"; Write-Success "MSVC: $linkDir" } } } }; Write-Step "Checking cargo installation..."; try { $cargoVersion = cargo --version 2>&1; if ($LASTEXITCODE -ne 0) { throw "Cargo not found" }; Write-Success "Cargo: $cargoVersion" } catch { Write-Fail "Cargo not installed: $_"; exit 1 }; Write-Step "Building with maximum optimization..."; try { $env:RUSTFLAGS = "-C target-cpu=native"; cargo build --release --manifest-path intellicrack-launcher/Cargo.toml 2>&1 | ForEach-Object { Write-Host "  $_" }; if ($LASTEXITCODE -ne 0) { throw "Cargo build failed with exit code $LASTEXITCODE" }; Write-Success "Build complete" } catch { Write-Fail "Build failed: $_"; exit 1 }; Write-Step "Validating build artifact..."; $exePath = "intellicrack-launcher\target\release\Intellicrack.exe"; if (-not (Test-Path $exePath)) { Write-Fail "Build artifact not found: $exePath"; exit 1 }; $exeSize = [math]::Round((Get-Item $exePath).Length / 1KB, 1); Write-Success "Artifact validated: Intellicrack.exe ($exeSize KB)"; Write-Step "Creating shortcut..."; try { $WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut("$PWD\Intellicrack.lnk"); $Shortcut.TargetPath = "$PWD\$exePath"; $Shortcut.IconLocation = "$PWD\intellicrack\assets\icon.ico"; $Shortcut.WorkingDirectory = "$PWD"; $Shortcut.Save(); if (-not (Test-Path "Intellicrack.lnk")) { throw "Shortcut creation failed" }; Write-Success "Shortcut created: Intellicrack.lnk" } catch { Write-Fail "Shortcut creation failed: $_"; exit 1 }; $elapsed = ((Get-Date) - $startTime).TotalSeconds; Write-Host "`n$e[1;32m=== Build Complete ===$e[0m $e[90m($("{0:N1}" -f $elapsed)s)$e[0m`n"

# Build Rust launcher in debug mode
build-rust-debug:
    @$ErrorActionPreference = 'Stop'; $e = [char]27; function Write-Step { param($msg) Write-Host "$e[36m[BUILD]$e[0m $msg" }; function Write-Success { param($msg) Write-Host "  $e[32m[OK]$e[0m $msg" }; function Write-Fail { param($msg) Write-Host "  $e[31m[FAIL]$e[0m $msg" }; $startTime = Get-Date; Write-Host "`n$e[1;36m=== Building Rust Launcher (Debug) ===$e[0m`n"; Write-Step "Configuring MSVC environment..."; $vsPath = & "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe" -latest -property installationPath 2>$null; if ($vsPath) { $msvcPath = Get-ChildItem "$vsPath\VC\Tools\MSVC" -Directory | Sort-Object Name -Descending | Select-Object -First 1; if ($msvcPath) { $linkDir = "$($msvcPath.FullName)\bin\Hostx64\x64"; $libDir = "$($msvcPath.FullName)\lib\x64"; $incDir = "$($msvcPath.FullName)\include"; if (Test-Path "$linkDir\link.exe") { $env:PATH = "$linkDir;$env:PATH"; $sdkVer = Get-ChildItem "${env:ProgramFiles(x86)}\Windows Kits\10\Lib" -Directory | Sort-Object Name -Descending | Select-Object -First 1; if ($sdkVer) { $sdkBase = "${env:ProgramFiles(x86)}\Windows Kits\10"; $env:LIB = "$libDir;$($sdkVer.FullName)\ucrt\x64;$($sdkVer.FullName)\um\x64"; $env:INCLUDE = "$incDir;$sdkBase\Include\$($sdkVer.Name)\ucrt;$sdkBase\Include\$($sdkVer.Name)\um;$sdkBase\Include\$($sdkVer.Name)\shared"; Write-Success "MSVC: $linkDir" } } } }; Write-Step "Building debug build..."; try { cargo build --manifest-path intellicrack-launcher/Cargo.toml 2>&1 | ForEach-Object { Write-Host "  $_" }; if ($LASTEXITCODE -ne 0) { throw "Cargo build failed with exit code $LASTEXITCODE" }; Write-Success "Build complete" } catch { Write-Fail "Build failed: $_"; exit 1 }; Write-Step "Validating build artifact..."; $exePath = "intellicrack-launcher\target\debug\Intellicrack.exe"; if (-not (Test-Path $exePath)) { Write-Fail "Build artifact not found: $exePath"; exit 1 }; $exeSize = [math]::Round((Get-Item $exePath).Length / 1KB, 1); Write-Success "Artifact validated: Intellicrack.exe ($exeSize KB)"; $elapsed = ((Get-Date) - $startTime).TotalSeconds; Write-Host "`n$e[1;32m=== Debug Build Complete ===$e[0m $e[90m($("{0:N1}" -f $elapsed)s)$e[0m`n"
...
kill:
    @$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator); if(-not $isAdmin){ Write-Host "Requesting administrator elevation..." -ForegroundColor Yellow; $script = '$procs = @(\"python\",\"pythonw\",\"cargo\",\"rustc\",\"rustfmt\",\"uv\",\"pixi\",\"yarn\",\"node\"); Write-Host \"\"; Write-Host \"=== Killing Dev Processes (Elevated) ===\" -ForegroundColor Cyan; Write-Host \"\"; $k=0; foreach($n in $procs){ $p=Get-Process -Name $n -EA SilentlyContinue; if($p){ $c=@($p).Count; $p | Stop-Process -Force -EA SilentlyContinue; Write-Host \"  [OK] $n ($c)\" -ForegroundColor Green; $k+=$c } else { Write-Host \"  [SKIP] $n\" -ForegroundColor DarkGray } }; Write-Host \"\"; Write-Host \"=== Done: $k killed ===\" -ForegroundColor Green; Write-Host \"\"; Read-Host \"Press Enter to close\"'; Start-Process pwsh -Verb RunAs -ArgumentList "-NoProfile","-Command",$script; exit 0 }; $procs = @("python","pythonw","cargo","rustc","rustfmt","uv","pixi","yarn","node"); Write-Host ""; Write-Host "=== Killing Dev Processes (Elevated) ===" -ForegroundColor Cyan; Write-Host ""; $k=0; foreach($n in $procs){ $p=Get-Process -Name $n -EA SilentlyContinue; if($p){ $c=@($p).Count; $p | Stop-Process -Force -EA SilentlyContinue; Write-Host "  [OK] $n ($c)" -ForegroundColor Green; $k+=$c } else { Write-Host "  [SKIP] $n" -ForegroundColor DarkGray } }; Write-Host ""; Write-Host "=== Done: $k killed ===" -ForegroundColor Green; Write-Host ""
