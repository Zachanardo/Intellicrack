...
git-commit:
    @$ErrorActionPreference = 'Stop'; $e = [char]27; function Write-Step { param($msg) Write-Host "$e[32m[GIT]$e[0m $msg" }; function Write-Success { param($msg) Write-Host "  $e[32m[OK]$e[0m $msg" }; function Write-Fail { param($msg) Write-Host "  $e[31m[FAIL]$e[0m $msg" }; $startTime = Get-Date; Write-Host "`n$e[1;32m=== WIP Commit ===$e[0m`n"; Write-Step "Running NUL cleanup..."; try { python scripts/clean_nul.py 2>&1 | Out-Null; Write-Success "NUL cleanup complete" } catch { Write-Success "NUL cleanup skipped" }; Write-Step "Committing and pushing..."; $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"; try { git add -A 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "git add failed" }; Write-Success "Changes staged" } catch { Write-Fail "Staging failed: $_"; exit 1 }; try { git commit --no-verify -m "WIP: $timestamp" 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "git commit failed" }; Write-Success "Committed: WIP: $timestamp" } catch { Write-Fail "Commit failed: $_"; exit 1 }; try { git push origin HEAD 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "git push failed" }; Write-Success "Pushed to origin" } catch { Write-Fail "Push failed: $_"; exit 1 }; $elapsed = ((Get-Date) - $startTime).TotalSeconds; Write-Host "`n$e[1;32m=== Commit Complete ===$e[0m $e[90m($("{0:N1}" -f $elapsed)s)$e[0m`n"
