...
class TestAnalysisStage:
    ...
    def test_analysis_stage_processes_binary_path(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = {"file_type": "PE", "arch": "x86"}
    ...
    def test_analysis_stage_handles_errors(self, tmp_path: Path) -> None:
        stage = AnalysisStage()
        input_data = PipelineData(content="/nonexistent/file.exe", metadata={}, format="text")

        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary", side_effect=FileNotFoundError("Not found")):
            output = stage.process(input_data)
    ...
    def test_analysis_stage_validates_input(self) -> None:
        ...
        invalid_input = "not a PipelineData object"
...
class TestPipelineExecution:
    ...
    def test_pipeline_single_stage(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = {"file_type": "PE"}
    ...
    def test_pipeline_multiple_stages(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = {"vulnerabilities": [{"type": "test"}]}
    ...
    def test_pipeline_error_handling(self) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary", side_effect=RuntimeError("Test error")):
...
class TestRealWorldPipelines:
    ...
    def test_vulnerability_analysis_pipeline(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = {"vulnerabilities": [{"type": "test", "severity": "high"}]}

            result = pipeline.execute(str(test_binary))

        assert output_file.exists()

    def test_protection_summary_pipeline(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = {"protections": {"aslr": True, "dep": True}}
    ...
    def test_csv_export_pipeline(self, tmp_path: Path) -> None:
        ...
        with patch("intellicrack.utils.analysis.binary_analysis.analyze_binary") as mock_analyze:
            mock_analyze.return_value = [{"function": "main", "risk": "low"}]
