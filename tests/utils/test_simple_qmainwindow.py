...
try:
    ...
    if HAS_PYQT:
        print("\nOK Creating QApplication...")
        app = QApplication(sys.argv)

        print("OK Creating QMainWindow instance...")
        window = QMainWindow()
        print(f"   Window type: {type(window)}")

        # Test methods
        try:
            print("OK Testing setGeometry...")
            window.setGeometry(100, 100, 800, 600)
            print("   setGeometry worked!")

            print("OK Testing setWindowTitle...")
            window.setWindowTitle("Test")
            print("   setWindowTitle worked!")

            print("OK All QMainWindow methods working!")

        except Exception as e:
            print(f"FAIL Method test failed: {e}")
    else:
        print("FAIL HAS_PYQT is False")
