...
const MESSAGE_TYPES = ['info', 'warning', 'error', 'status', 'bypass', 'success', 'detection', 'notification'];
...
function recordTestResult(testName, passed, latency, details) {
    ...
    if (latency !== undefined) {
        ...
        TEST_RESULTS.averageLatency = ((TEST_RESULTS.averageLatency * (TEST_RESULTS.totalTests - 1)) + latency) / TEST_RESULTS.totalTests;
    }
...
function runBasicMessageTests() {
    ...
    MESSAGE_TYPES.forEach((messageType, index) => {
        ...
        recordTestResult(`basic_${messageType}`, true, latency, `${messageType} message sent successfully`);
    });
}
...
function runPerformanceTests() {
    ...
    console.log(`[TEST] Performance test completed: ${TEST_RESULTS.messagesPerSecond.toFixed(2)} messages/sec`);
}
    ...
    for (let batch = 0; batch < concurrentBatches; batch++) {
        ...
        for (let msg = 0; msg < messagesPerBatch; msg++) {
            ...
            try {
                ...
                recordTestResult(`concurrency_b${batch}_m${msg}`, true, latency, 'Concurrent message processed');
            } catch (error) {
                recordTestResult(`concurrency_b${batch}_m${msg}`, false, 0, `Concurrency failure: ${error.message}`);
                TEST_RESULTS.concurrencyFailures++;
            }
        }
    }

    console.log(`[TEST] Concurrency test completed: ${TEST_RESULTS.concurrencyFailures} failures detected`);
}
...
function runErrorInjectionTests() {
    ...
    const invalidMessages = [
        ...
        { type: 'info', target: 'error_test', action: 'oversized_test', huge_payload: generateTestData(10000) },
        null, // Null message
    ...
    invalidMessages.forEach((invalidMsg, index) => {
        try {
            if (invalidMsg === null || invalidMsg === undefined) {
                recordTestResult(`error_injection_${index}`, true, 0, 'Correctly handled null/undefined message');
                return;
            }

            const validation = validateMessage(invalidMsg);
            if (!validation.valid) {
                recordTestResult(`error_injection_${index}`, true, 0, `Correctly rejected invalid message: ${validation.error}`);
            } else {
                send(invalidMsg);
                recordTestResult(`error_injection_${index}`, false, 0, 'Invalid message was not rejected');
                TEST_RESULTS.validationFailures++;
            }
        } catch (error) {
            recordTestResult(`error_injection_${index}`, true, 0, `Correctly threw error for invalid message: ${error.message}`);
        }
    });
}
...
function runTimingAnalysisTests() {
    ...
    timingTests.forEach(test => {
        ...
        const withinExpectedRange = latency >= MESSAGE_VALIDATION_RULES.min_response_time_ms &&
                                   latency <= MESSAGE_VALIDATION_RULES.max_response_time_ms;

        recordTestResult(`timing_${test.name}`, withinExpectedRange, latency,
...
function runComprehensiveTestSuite() {
    ...
    console.log(`[TEST FRAMEWORK] Test suite completed: ${TEST_RESULTS.passedTests}/${TEST_RESULTS.totalTests} tests passed (${successRate.toFixed(2)}%)`);
}
...
function generateRecommendations(successRate) {
    ...
    if (successRate < 95.0) {
        recommendations.push('Success rate below production threshold (95%). Review failed test details.');
    }

    if (TEST_RESULTS.validationFailures > 0) {
        recommendations.push('Message validation failures detected. Review message format standards.');
    }

    if (TEST_RESULTS.concurrencyFailures > 0) {
        recommendations.push('Concurrency failures detected. Review thread safety and message queuing.');
    }

    if (TEST_RESULTS.maxLatency > 500) {
        recommendations.push('High latency detected (>500ms). Optimize message processing pipeline.');
    }

    if (TEST_RESULTS.messagesPerSecond < 100) {
        recommendations.push('Low throughput detected (<100 msg/sec). Optimize message handling performance.');
    }
