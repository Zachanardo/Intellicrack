...
class FakeFileDialog:
    ...
    def getOpenFileName(self, *args: Any, **kwargs: Any) -> tuple[str, str]:
        return (self.selected_file, "")


class FakeFile:
    """Test double for file operations."""
...
class FakeBypassGenerator:
    """Test double for bypass script generator."""
...
class TestResultExport:
    ...
    def test_export_results_to_text_file(self, test_script_dialog: TestScriptDialog, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            file_path = f.name
            fake_dialog.selected_file = file_path

        try:
            monkeypatch.setattr("builtins.open", lambda *args, **kwargs: fake_file)
            monkeypatch.setattr(test_script_dialog, "QFileDialog", fake_dialog)

            test_script_dialog.export_results()

            assert fake_file.content != "" or fake_file.closed
        finally:
...
class TestScriptGeneratorWorker:
    ...
    def test_worker_generates_bypass_script(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        fake_generator = FakeBypassGenerator()

        def fake_generate(*args: Any, **kwargs: Any) -> dict[str, str]:
            return fake_generator.generate(*args, **kwargs)

        monkeypatch.setattr("intellicrack.utils.exploitation.generate_bypass_script", fake_generate)
        worker.run()
        assert result
