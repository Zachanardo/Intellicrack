...
def test_cache_directory_structure_creation(tmp_path: Path) -> None:
    ...
    with patch("intellicrack.core.analysis.incremental_analyzer.get_config") as mock_cfg:
        mock_cfg.return_value.get.return_value = str(cache_base)
...
def test_real_cache_write_and_read_cycle(
    protected_binary: Path, isolated_cache_dir: Path
) -> None:
    """Complete write and read cycle with real cache files."""
    from unittest.mock import patch

    with patch("intellicrack.core.analysis.incremental_analyzer.get_config") as mock_cfg:
        mock_cfg.return_value.get.return_value = str(isolated_cache_dir.parent)
...
def test_multiple_binaries_separate_caches(tmp_path: Path) -> None:
...
def test_cache_path_collision_resistance(tmp_path: Path) -> None:
