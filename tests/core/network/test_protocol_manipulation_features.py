...
\r\n"""
    ...
    def _decode_protocol_payload(self, encoded_payload: str, encoding_type: str) -> str:
        ...
        if encoding_type == 'BASE64':
            ...
            result: list[str] = []
            idx = 0
            while idx < len(encoded_payload):
                if encoded_payload[idx] == '%' and idx + 2 < len(encoded_payload):
                    hex_str = encoded_payload[idx + 1:idx + 3]
                    try:
                        result.append(chr(int(hex_str, 16)))
                        idx += 3
                    except ValueError:
                        result.append(encoded_payload[idx])
                        idx += 1
                else:
                    result.append(encoded_payload[idx])
                    idx += 1
            ...
            return encoded_payload.encode().decode('unicode_escape')
        else:
