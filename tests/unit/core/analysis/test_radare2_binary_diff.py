...
class TestR2BinaryDiffFunctionAnalysis(unittest.TestCase):
    ...
    def test_compare_functions_detects_new_functions(self):
        ...
        self.assertTrue(
            any("new" in str(key).lower() or "add" in str(key).lower() for key in function_result)
            or any("new_security_check" in str(value) for value in function_result.values() if isinstance(value, str))
        )
    ...
    def test_compare_functions_detects_modified_functions(self):
        ...
        self.assertTrue(
            any("modif" in str(key).lower() or "chang" in str(key).lower() for key in function_result)
