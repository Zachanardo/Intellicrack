...
class TestOpenAIModelListing:
    ...
    async def test_model_info_has_correct_provider(
        ...
        for model in models:
            assert model.provider == ProviderName.OPENAI, (
                f"Expected OPENAI provider, got {model.provider}"
            )
    ...
    async def test_models_are_chat_models(
        ...
        for model in models:
            has_chat_prefix = any(
                model.id.startswith(prefix) for prefix in chat_prefixes
            )
            assert has_chat_prefix, (
                f"Model {model.id} doesn't appear to be a chat model"
            )
