...
class TestRootCAGeneration:
    ...
    def test_generate_root_ca_returns_valid_certificate_and_key(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
    ...
    def test_root_ca_has_correct_subject_fields(self) -> None:
        ...
        subject_attrs: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in root_cert.subject
        }
...
class TestIntermediateCAGeneration:
    ...
    def test_intermediate_ca_has_correct_subject_fields(self) -> None:
        ...
        subject_attrs: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in intermediate_cert.subject
        }
...
class TestLeafCertificateGeneration:
    ...
    def test_leaf_cert_has_correct_common_name(self) -> None:
        ...
        subject_attrs: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in leaf_cert.subject
        }
    ...
    def test_leaf_cert_has_subject_alternative_name_with_domain(self) -> None:
        ...
        dns_names: List[str] = [
            cast(str, name) for name in san.get_values_for_type(x509.DNSName)
        ]
...
class TestFullChainGeneration:
    ...
    def test_generate_full_chain_returns_complete_chain(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("example.com")
    ...
    def test_full_chain_certificates_form_valid_hierarchy(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("secure.example.com")
    ...
    def test_full_chain_all_signatures_verify(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("api.example.com")
    ...
    def test_full_chain_leaf_cert_matches_requested_domain(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        domain: str = "test.intellicrack.local"
        chain: CertificateChain = generator.generate_full_chain(domain)

        subject_attrs: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in chain.leaf_cert.subject
        }
        assert subject_attrs[NameOID.COMMON_NAME] == domain
    ...
    def test_full_chain_generates_unique_certificates(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain1: CertificateChain = generator.generate_full_chain("example.com")
        chain2: CertificateChain = generator.generate_full_chain("example.com")
...
class TestCertificateExport:
    ...
    def test_export_chain_pem_produces_valid_pem_format(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("example.com")
        pem_chain: str = generator.export_chain_pem(chain)
    ...
    def test_export_chain_pem_orders_certificates_correctly(self) -> None:
        ...
        leaf_pem: str = chain.leaf_cert.public_bytes(serialization.Encoding.PEM).decode()
        intermediate_pem: str = chain.intermediate_cert.public_bytes(
            serialization.Encoding.PEM
        ).decode()
        root_pem: str = chain.root_cert.public_bytes(serialization.Encoding.PEM).decode()

        leaf_pos: int = pem_chain.index(leaf_pem)
        intermediate_pos: int = pem_chain.index(intermediate_pem)
        root_pos: int = pem_chain.index(root_pem)
    ...
    def test_export_chain_pem_can_be_loaded_back(self) -> None:
        ...
        cert_blocks: List[str] = pem_chain.split("-----BEGIN CERTIFICATE-----")[1:]
        loaded_certs: List[x509.Certificate] = []
        for block in cert_blocks:
            cert_pem: str = f"-----BEGIN CERTIFICATE-----{block}"
    ...
    def test_export_cert_der_produces_valid_der_bytes(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        root_cert, _ = generator.generate_root_ca()
        der_bytes: bytes = generator.export_cert_der(root_cert)

        assert isinstance(der_bytes, bytes)
        assert len(der_bytes) > 100

        loaded_cert: x509.Certificate = x509.load_der_x509_certificate(der_bytes)
    ...
    def test_export_private_key_pem_produces_valid_key(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        _, root_key = generator.generate_root_ca()
        key_pem: str = generator.export_private_key_pem(root_key)
    ...
    def test_export_public_key_pem_produces_valid_key(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        root_cert, root_key = generator.generate_root_ca()
        public_key: rsa.RSAPublicKey = root_key.public_key()
        pub_key_pem: str = generator.export_public_key_pem(public_key)
    ...
    def test_exported_private_key_can_decrypt_data_encrypted_with_public_key(
        ...
        public_key: rsa.RSAPublicKey = leaf_key.public_key()

        plaintext: bytes = b"Test data for encryption"
        ...
        key_pem: str = generator.export_private_key_pem(leaf_key)
        loaded_key = load_pem_private_key(key_pem.encode(), password=None)
        assert isinstance(loaded_key, rsa.RSAPrivateKey)

        decrypted: bytes = loaded_key.decrypt(
            ciphertext,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                algorithm=hashes.SHA256(),
...
class TestCertificatePinningBypass:
    ...
    def test_leaf_cert_public_key_can_be_extracted_for_pinning(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("pinned.example.com")
    ...
    def test_leaf_cert_spki_sha256_hash_matches_expected_pin_format(self) -> None:
        ...
        public_key = chain.leaf_cert.public_key()
        spki_bytes: bytes = public_key.public_bytes(
            encoding=serialization.Encoding.DER,
            format=serialization.PublicFormat.SubjectPublicKeyInfo,
        )

        pin_sha256: bytes = hashlib.sha256(spki_bytes).digest()
        assert len(pin_sha256) == 32

        import base64

        pin_base64: str = base64.b64encode(pin_sha256).decode()
    ...
    def test_root_ca_can_be_used_to_sign_arbitrary_leaf_certs(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("original.example.com")
    ...
    def test_multiple_leaf_certs_can_share_same_ca_chain(self) -> None:
        ...
        domains: List[str] = ["api.example.com", "cdn.example.com", "secure.example.com"]
        leaf_certs: List[x509.Certificate] = []
    ...
    def test_generated_chain_can_be_used_for_https_server_configuration(
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("server.example.com")

        leaf_cert_pem: str = chain.leaf_cert.public_bytes(
            serialization.Encoding.PEM
        ).decode()
        leaf_key_pem: str = generator.export_private_key_pem(chain.leaf_key)
        chain_pem: str = generator.export_chain_pem(chain)
...
class TestCryptographicOperations:
    ...
    def test_leaf_private_key_can_sign_data(self) -> None:
        ...
        data: bytes = b"Test data to be signed"
        signature: bytes = chain.leaf_key.sign(
            data,
            padding.PSS(
    ...
    def test_leaf_public_key_can_verify_signature_from_private_key(self) -> None:
        ...
        data: bytes = b"Data to sign and verify"
        signature: bytes = chain.leaf_key.sign(
            data,
            padding.PSS(
                mgf=padding.MGF1(hashes.SHA256()),
    ...
    def test_private_key_and_certificate_public_key_match(self) -> None:
        ...
        private_key_public: rsa.RSAPublicKey = chain.leaf_key.public_key()
...
class TestEdgeCasesAndVariations:
    ...
    def test_subdomain_certificate_generation(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        domain: str = "deeply.nested.subdomain.example.com"
        chain: CertificateChain = generator.generate_full_chain(domain)
    ...
    def test_international_domain_name_support(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        unicode_domain: str = "例え.jp"
        a_label_domain: str = unicode_domain.encode('idna').decode('ascii')
        chain: CertificateChain = generator.generate_full_chain(a_label_domain)

        subject_attrs: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in chain.leaf_cert.subject
        }
        cn: str = subject_attrs[NameOID.COMMON_NAME]
    ...
    def test_domain_with_hyphens(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        domain: str = "my-test-domain.example.com"
        chain: CertificateChain = generator.generate_full_chain(domain)
    ...
    def test_very_long_domain_name(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        domain: str = "very.long.subdomain.name.with.many.levels.example.com"
        chain: CertificateChain = generator.generate_full_chain(domain)
    ...
    def test_uppercase_domain_name(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        domain: str = "UPPERCASE.EXAMPLE.COM"
        chain: CertificateChain = generator.generate_full_chain(domain)
...
class TestRealWorldUsageScenarios:
    ...
    def test_mitm_proxy_certificate_chain_export(self, tmp_path: Path) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("target.example.com")
        ...
        leaf_pem: str = chain.leaf_cert.public_bytes(serialization.Encoding.PEM).decode()
        key_pem: str = generator.export_private_key_pem(chain.leaf_key)
        root_pem: str = chain.root_cert.public_bytes(serialization.Encoding.PEM).decode()
        ...
        loaded_cert: x509.Certificate = x509.load_pem_x509_certificate(cert_file.read_bytes())
        loaded_key = load_pem_private_key(key_file.read_bytes(), password=None)
        loaded_ca: x509.Certificate = x509.load_pem_x509_certificate(ca_file.read_bytes())
    ...
    def test_multiple_domain_mitm_scenario(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        base_chain: CertificateChain = generator.generate_full_chain("base.example.com")
        ...
        for domain, (cert, key) in generated_certs.items():
            subject_attrs: Dict[ObjectIdentifier, str] = {
                attr.oid: cast(str, attr.value) for attr in cert.subject
            }
            assert subject_attrs[NameOID.COMMON_NAME] == domain
    ...
    def test_certificate_trust_chain_verification_simulation(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("verified.example.com")
        ...
        root_self_signed: bool = chain.root_cert.issuer == chain.root_cert.subject
    ...
    def test_certificate_renewal_workflow(self) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        original_chain: CertificateChain = generator.generate_full_chain("renewable.example.com")
        ...
        subject_attrs_original: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in original_chain.leaf_cert.subject
        }
        subject_attrs_renewed: Dict[ObjectIdentifier, str] = {
            attr.oid: cast(str, attr.value) for attr in renewed_leaf.subject
        }
        assert (
    ...
    def test_export_for_different_server_formats(self, tmp_path: Path) -> None:
        ...
        generator: CertificateChainGenerator = CertificateChainGenerator()
        chain: CertificateChain = generator.generate_full_chain("multi-format.example.com")

        pem_cert: bytes = chain.leaf_cert.public_bytes(serialization.Encoding.PEM)
        der_cert: bytes = generator.export_cert_der(chain.leaf_cert)
        pem_key: str = generator.export_private_key_pem(chain.leaf_key)
        full_chain_pem: str = generator.export_chain_pem(chain)
        ...
        loaded_pem: x509.Certificate = x509.load_pem_x509_certificate((tmp_path / "cert.pem").read_bytes())
        loaded_der: x509.Certificate = x509.load_der_x509_certificate((tmp_path / "cert.der").read_bytes())
