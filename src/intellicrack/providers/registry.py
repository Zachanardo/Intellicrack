...
class ProviderRegistry:
    ...
    def get_or_raise(self, name: ProviderName) -> LLMProviderBase:
        ...
        if provider is None:
            raise ProviderError(f"Provider {name.value} is not registered")
        return provider
    ...
    async def connect_provider(
        ...
        Raises:
            ProviderError: If provider not registered or connection fails.
        """
        ...
        if credentials is None:
            raise ProviderError(f"No credentials available for {name.value}")
        ...
        try:
            ...
            self._logger.error("Failed to connect to %s: %s", name.value, e)
            raise
    ...
    async def connect_all(
        ...
        async def connect_one(name: ProviderName) -> tuple[ProviderName, bool]:
            try:
                ...
                self._logger.warning(
                    "Failed to connect %s: %s", name.value, str(e)
                )
                return name, False
        ...
        for name, provider in self._providers.items():
            if provider.is_connected:
                try:
                    ...
                    self._logger.warning(
                        "Failed to get models from %s: %s", name.value, str(e)
                    )
                    models[name] = []
            else:
                models[name] = []
    ...
    def set_active(self, name: ProviderName) -> None:
        ...
        if not provider.is_connected:
            raise ProviderError(f"Provider {name.value} is not connected")
...
async def get_active_provider() -> LLMProviderBase:
    """Get the currently active provider.
