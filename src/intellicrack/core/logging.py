...
class IntellicrackLogger:
    ...
    def __init__(self, name: str = "intellicrack") -> None:
        ...
        self._console_handler: logging.StreamHandler[logging.StreamHandler] | None = (
            None
        )
...
def setup_logging(config: LogConfig) -> IntellicrackLogger:
    ...
    from .config import LogConfig as LogConfigType

    if not isinstance(config, LogConfigType):
        raise TypeError(f"Expected LogConfig, got {type(config).__name__}")
