...
class Orchestrator:
    ...
    async def start_session(
        ...
        if provider_instance is None or not provider_instance.is_connected:
            raise ValueError(f"Provider not available: {provider.value}")
    ...
    async def _load_binary(self, path: Path) -> BinaryInfo:
        ...
        return await binary_bridge.load_binary(path)
    ...
    async def _call_llm(
        ...
        if self._config.stream_responses and self._on_stream_chunk:
    ...
    async def set_active_binary(self, index: int) -> None:
        ...
        if index < 0 or index >= len(self._current_session.binaries):
            raise IndexError(f"Binary index out of range: {index}")
