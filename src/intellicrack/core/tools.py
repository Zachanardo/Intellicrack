...
class ToolRegistry:
    ...
    async def initialize_tool(self, name: ToolName) -> bool:
        ...
        try:
            ...
            _logger.error("Failed to initialize tool %s: %s", name.value, e)
            return False
    ...
    def get_binary_bridge(self) -> BinaryBridge:
        ...
        if bridge is None or not isinstance(bridge, BinaryBridge):
            raise ToolError("Binary bridge not available")
        return bridge
    ...
    def get_process_bridge(self) -> ProcessBridge:
        ...
        if bridge is None or not isinstance(bridge, ProcessBridge):
            raise ToolError("Process bridge not available")
        return bridge
    ...
    def get_frida_bridge(self) -> FridaBridge:
        ...
        if bridge is None or not isinstance(bridge, FridaBridge):
            raise ToolError("Frida bridge not available")
        return bridge
    ...
    def get_ghidra_bridge(self) -> GhidraBridge:
        ...
        if bridge is None or not isinstance(bridge, GhidraBridge):
            raise ToolError("Ghidra bridge not available")
        return bridge
    ...
    def get_radare2_bridge(self) -> Radare2Bridge:
        ...
        if bridge is None or not isinstance(bridge, Radare2Bridge):
            raise ToolError("radare2 bridge not available")
        return bridge
    ...
    def get_x64dbg_bridge(self) -> X64DbgBridge:
        ...
        if bridge is None or not isinstance(bridge, X64DbgBridge):
            raise ToolError("x64dbg bridge not available")
        return bridge
    ...
    async def execute_tool_call(
        ...
        try:
            ...
            raise ToolError(f"Unknown tool: {tool_name}") from None

        bridge = self._bridges.get(tool_enum)
        if bridge is None:
            raise ToolError(f"Tool not registered: {tool_name}")

        method = getattr(bridge, function_name, None)
        if method is None:
            raise ToolError(f"Unknown function: {tool_name}.{function_name}")

        if not callable(method):
            raise ToolError(f"Not callable: {tool_name}.{function_name}")
        ...
        try:
            ...
            _logger.error("Tool call failed: %s.%s - %s", tool_name, function_name, e)
            raise ToolError(f"Tool call failed: {e}") from e
