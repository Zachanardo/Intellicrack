...
class Config:
    ...
    def _from_dict(cls, data: dict[str, Any]) -> Config:
        ...
        for name_str, prov_data in providers_data.items():
            ...
            if provider_name in providers:
                ...
                providers[provider_name] = ProviderConfig(
                    enabled=prov_data.get("enabled", base.enabled),
                    api_base=prov_data.get("api_base", base.api_base),
                    default_model=prov_data.get("default_model", base.default_model),
                    timeout_seconds=prov_data.get(
                        "timeout_seconds", base.timeout_seconds
                    ),
                    max_retries=prov_data.get("max_retries", base.max_retries),
                )
        ...
        for name_str, tool_data in tools_data.items():
            ...
            if tool_name in tools:
                ...
                tool_path = Path(path_str) if path_str else base.path
                tools[tool_name] = ToolConfig(
                    enabled=tool_data.get("enabled", base.enabled),
                    path=tool_path,
                    auto_install=tool_data.get("auto_install", base.auto_install),
    ...
    def _to_dict(self) -> dict[str, Any]:
        ...
        for name, prov_config in self.providers.items():
            ...
            data["providers"][name.value] = prov_dict
        ...
        for name, tool_config in self.tools.items():
            ...
            data["tools"][name.value] = tool_dict
