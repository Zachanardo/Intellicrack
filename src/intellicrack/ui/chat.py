...
class MessageBubble(QFrame):
    ...
    def _create_tool_call_widget(self, call: ToolCall) -> QFrame:
        ...
        if call.arguments:
            ...
            if len(args_text) > 100:
                args_text = args_text[:97] + "..."
            args_label = QLabel(args_text)
            args_label.setFont(QFont("JetBrains Mono", 8))
            args_label.setStyleSheet("color: #9CDCFE;")
            args_label.setWordWrap(True)
            layout.addWidget(args_label)

        return frame

    def _create_tool_result_widget(self, result: ToolResult) -> QFrame:
        ...
        color = "#6A9955" if result.success else "#F44747"
        ...
        if result.error:
            ...
            result_label.setStyleSheet("color: #D4D4D4;")
