...
SandboxType = Literal["windows", "docker", "qemu"]
...
class SandboxManager:
    ...
    async def create(
        ...
        async with self._lock:
            ...
            if not await sandbox.is_available():
                raise SandboxError(f"Sandbox type not available: {sandbox_type}")
    ...
    async def destroy(self, instance_id: str) -> None:
        ...
        async with self._lock:
            ...
            if instance is None:
                raise SandboxError(f"Sandbox instance not found: {instance_id}")
    ...
    async def run_binary(
        ...
        try:
            ...
            _logger.error("Binary execution failed in sandbox %s: %s", instance.id, e)
            raise
    ...
    async def _find_oldest_idle(self) -> SandboxInstance | None:
        ...
        for instance in self._instances.values():
            if instance.state.status == "running":
                if oldest_time is None or instance.last_used < oldest_time:
                    oldest = instance
