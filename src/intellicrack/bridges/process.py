...
class ProcessBridge(ToolBridgeBase):
    ...
    async def initialize(self, tool_path: Path | None = None) -> None:
        ...
        try:
            ...
            _logger.error("Failed to initialize process bridge: %s", e)
    ...
    async def list_processes(
        ...
        if self._kernel32 is None:
            raise ToolError("Kernel32 not available")
        ...
        if snapshot == -1:
            raise ToolError("Failed to create process snapshot")
    ...
    async def open_process(
        ...
        if not handle:
            ...
            raise ToolError(f"OpenProcess failed for PID {pid}: error {error}")
    ...
    async def terminate(self, pid: int | None = None) -> bool:
        ...
        if pid is not None:
            ...
            if not handle:
                raise ToolError(f"Cannot open process {pid} for termination")
            close_handle = True
        else:
        ...
        try:
            ...
            if not result:
                raise ToolError("TerminateProcess failed")
    ...
    async def suspend(self, pid: int | None = None) -> bool:
        ...
        if target_pid is None:
            raise ToolError("No process specified")
    ...
    async def read_memory(self, address: int, size: int) -> bytes:
        ...
        if not result:
            error = ctypes.get_last_error()
            raise ToolError(f"ReadProcessMemory failed at 0x{address:X}: error {error}")
    ...
    async def write_memory(self, address: int, data: bytes) -> int:
        ...
        if not result:
            error = ctypes.get_last_error()
            raise ToolError(f"WriteProcessMemory failed at 0x{address:X}: error {error}")
    ...
    async def allocate(
        ...
        if not address:
            error = ctypes.get_last_error()
            raise ToolError(f"VirtualAllocEx failed: error {error}")
    ...
    async def free(self, address: int) -> bool:
        ...
        if not result:
            error = ctypes.get_last_error()
            raise ToolError(f"VirtualFreeEx failed: error {error}")
    ...
    async def protect(
        ...
        if not result:
            error = ctypes.get_last_error()
            raise ToolError(f"VirtualProtectEx failed: error {error}")
    ...
    async def inject_dll(self, dll_path: str) -> bool:
        ...
        if not dll_path_resolved.exists():
            raise ToolError(f"DLL not found: {dll_path}")
        ...
        try:
            ...
            if not kernel32_handle:
                raise ToolError("Failed to get kernel32 handle")
            ...
            if not load_library_addr:
                raise ToolError("Failed to get LoadLibraryA address")
            ...
            if not thread_handle:
                error = ctypes.get_last_error()
                raise ToolError(f"CreateRemoteThread failed: error {error}")
