...
#AutoIt3Wrapper_Icon=Intellicrack.ico
#AutoIt3Wrapper_Outfile_x64=AdobeInjector.exe
#AutoIt3Wrapper_Res_Comment=Adobe Injector
#AutoIt3Wrapper_Res_CompanyName=Intellicrack
#AutoIt3Wrapper_Res_Description=Adobe Injector
#AutoIt3Wrapper_Res_Fileversion=1.2.0
#AutoIt3Wrapper_Res_LegalCopyright=Intellicrack 2025
#AutoIt3Wrapper_Res_LegalTradeMarks=Intellicrack
#AutoIt3Wrapper_Res_ProductName=Adobe Injector
...
Global $g_AppWndTitle = "Adobe Injector v" & $g_Version
Global $g_AppVersion = "Intellicrack"
	...
	Select
		...
		Case $idMsg = $idBtnCure
			...
			If $bFoundAcro32 = True Then
				MsgBox($MB_SYSTEMMODAL, "Information", "Adobe Injector does not patch the x32 bit version of Acrobat. Please use the x64 bit version of Acrobat.")
				LogWrite(1, "Adobe Injector does not patch the x32 bit version of Acrobat. Please use the x64 bit version of Acrobat.")
			EndIf
			If $bFoundGenericARM = True Then
				MsgBox($MB_SYSTEMMODAL, "Information", "This build does not support ARM binaries, only x64.")
				LogWrite(1, "This build does not support ARM binaries, only x64.")
			EndIf
		...
		Case $idMsg = $idBtnRestore
			GUICtrlSetData($idLog, "Activity Log" & @CRLF & "- - - - - - - - - - -" & @CRLF & @CRLF & "Adobe Injector Version: " & $g_Version & "" & @CRLF & "Config Version: " & $ConfigVerVar & "" & @CRLF)
			ToggleLog(0)
			GUICtrlSetState($idListview, 128)
			GUICtrlSetState($idBtnDeselectAll, 128)
			GUICtrlSetState($idButtonSearch, 128)
		...
		Case $idMsg = $idBtnRuntimeInfo
			ShowInfoPopup("Select apps may pack the RuntimeInstaller.dll with UPX causing patching to fail. Adobe Injector can unpack these files so they can then be patched." & @CRLF & @CRLF & @CRLF & @CRLF & _
		...
		Case $idMsg = $idBtnWintrustInfo
			ShowInfoPopup("Avoid popups by ' trusting' each app. Uses a modified DLL + registry edit for allowing DLL redirection. Trust/Untrust each app or add/remove the reg key as needed. Reg key is auto-added when trusting apps.")
	EndSelect
WEnd
...
Func MainGui()
	...
	$g_idHyperlinkMain = GUICtrlCreateLabel("", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$g_idHyperlinkOptions = GUICtrlCreateLabel("", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	GUICtrlSetTip(-1, "Enable/Disable all Adobe Injector firewall rules")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	$idBtnRemoveFW = GUICtrlCreateButton("Remove Rules", 300, 140, 140, 30)
	GUICtrlSetTip(-1, "Remove all Adobe Injector firewall rules")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	...
	GUICtrlSetTip(-1, "Remove hosts added by Adobe Injector")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	...
	$g_idHyperlinkPopup = GUICtrlCreateLabel("", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$g_idHyperlinkLog = GUICtrlCreateLabel("", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
...
Func FillListViewWithInfo()
	...
	_GUICtrlListView_AddSubItem($idListview, 0, "", 1)
	_GUICtrlListView_AddSubItem($idListview, 1, "Adobe Injector", 1)
	_GUICtrlListView_AddSubItem($idListview, 2, "Intellicrack", 1)
...
Func MyGlobalPatternPatch($MyFileToPatch, $MyArrayToPatch)
	...
	If $iRows > 0 Then
		...
		LogWrite(1, "File patched by Adobe Injector " & $g_Version & " + config " & $ConfigVerVar)
		If $bEnableMD5 = 1 Then
			_Crypt_Startup()
...
Func EnableAllFWRules()
	MemoWrite("Enabling all Adobe Injector firewall rules...")
	LogWrite(1, "Starting process to enable all Adobe Injector firewall rules.")
	...
	If $iRuleCount = 0 Then
		MemoWrite("No Adobe Injector firewall rules found to enable.")
		LogWrite(1, "No Adobe Injector firewall rules found.")
		LogWrite(1, "Enable rules process completed." & @CRLF)
		ToggleLog(1)
		Return
	EndIf
	...
	Else
		MemoWrite("All Adobe Injector firewall rules enabled successfully.")
		LogWrite(1, "All Adobe Injector firewall rules enabled successfully.")
	EndIf
...
Func DisableAllFWRules()
	MemoWrite("Disabling all Adobe Injector firewall rules...")
	LogWrite(1, "Starting process to disable all Adobe Injector firewall rules.")
	...
	If $iRuleCount = 0 Then
		MemoWrite("No Adobe Injector firewall rules found to disable.")
		LogWrite(1, "No Adobe Injector firewall rules found.")
		LogWrite(1, "Disable rules process completed." & @CRLF)
		ToggleLog(1)
		Return
	...
	Else
		MemoWrite("All Adobe Injector firewall rules disabled successfully.")
		LogWrite(1, "All Adobe Injector firewall rules disabled successfully.")
	EndIf
