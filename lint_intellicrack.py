...
class IntellicrackLinter:
    ...
    def scan_project(self) -> None:
        ...
        for i, file_path in enumerate(python_files, 1):
            print(f"Analyzing {i}/{len(python_files)}: {file_path.relative_to(self.project_root)}")
            try:
    ...
    def _create_markdown_report(self) -> str:
        ...
        for file_path in sorted(issues_by_file.keys()):
            ...
            for issue in issues:
                ...
                report.append(f"- **Line {issue.line_number}** {severity_emoji} {issue.message}")
                if issue.suggestion:
        ...
        no_docstring = [m for m in self.modules.values() if not m.has_docstring]
        if no_docstring:
            report.append("### Files Missing Module Docstrings")
            report.append("")
            for module in sorted(no_docstring, key=lambda x: x.file_path):
                rel_path = Path(module.file_path).relative_to(self.project_root)
                report.append(f"- `{rel_path}`")
            report.append("")
            
        # Files without main guard
        no_main_guard = [m for m in self.modules.values() if not m.has_main_guard and m.line_count > 50]
        if no_main_guard:
        ...
        report.append("")
        report.append("1. **Fix Syntax Errors**: Address all syntax errors before other improvements")
        report.append("2. **Add Docstrings**: Add module docstrings to improve code documentation")
        report.append("3. **Type Hints**: Add type hints to improve code clarity and catch errors")
        report.append("4. **Code Formatting**: Use black or similar formatter for consistent code style")
        report.append("5. **Import Organization**: Organize imports consistently (stdlib, third-party, local)")
        report.append("6. **Error Handling**: Add proper exception handling where missing")
        report.append("7. **Testing**: Add unit tests for modules with high complexity")
        report.append("")
