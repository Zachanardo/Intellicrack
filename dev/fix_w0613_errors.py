...
FILES_TO_FIX = [
    ...
    ('intellicrack/core/network/cloud_license_hooker.py', [
        ...
        (391, 'def _generate_rlm_response(self, service, request):', ['service', 'request']),
    ...
    ('intellicrack/hexview/ai_bridge.py', [
        ...
        (1133, 'def analyze_pattern(self, pattern_data, known_patterns=None):', 'known_patterns'),
    ]),
    ('intellicrack/hexview/hex_commands.py', [
        (78, 'def __eq__(self, other):', 'other'),
    ]),
    ('intellicrack/hexview/hex_renderer.py', [
        (92, 'def setup_display(self, start_offset=0, highlight_ranges=None):', 'highlight_ranges'),
    ...
    ('intellicrack/hexview/integration.py', [
        (27, 'def wrapper_ai_binary_analyze(*args, **kwargs):', ['args', 'kwargs']),
        (29, 'def wrapper_ai_binary_pattern_search(*args, **kwargs):', ['args', 'kwargs']),
        (31, 'def wrapper_ai_binary_edit_suggest(*args, **kwargs):', ['args', 'kwargs']),
    ]),
    ...
    ('intellicrack/plugins/__init__.py', [
        ...
        (105, 'def run_frida_plugin_from_file(*args, **kwargs):', ['args', 'kwargs']),
        (106, 'def run_ghidra_plugin_from_file(*args, **kwargs):', ['args', 'kwargs']),
        (107, 'def create_sample_plugins(*args, **kwargs):', ['args', 'kwargs']),
        (108, 'def run_plugin_in_sandbox(*args, **kwargs):', ['args', 'kwargs']),
    ...
    ('intellicrack/ui/main_app.py', [
        ...
        (224, 'def run_analysis(self, app, *args, **kwargs):', ['app', 'args', 'kwargs']),
        (233, 'def open_settings(self, app, *args, **kwargs):', ['app', 'args', 'kwargs']),
        (236, 'def open_ai_dialog(self, app, *args, **kwargs):', ['app', 'args', 'kwargs']),
    ]),
]
        ...
        for line_num, signature, unused_args in fixes:
            ...
            if idx < len(lines):
                ...
                if line.rstrip().endswith(':'):
                    lines[idx] = line.rstrip() + '  # pylint: disable=unused-argument\n'
                else:
                    # For multi-line signatures, find the colon
                    for i in range(idx, min(idx + 5, len(lines))):
                        if lines[i].rstrip().endswith(':'):
                            lines[i] = lines[i].rstrip() + '  # pylint: disable=unused-argument\n'
                            break
