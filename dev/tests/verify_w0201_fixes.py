...
def check_hasattr_usage(file_path: str, modified_attrs: Set[str]) -> List[str]:
    ...
    try:
        ...
        for attr in modified_attrs:
            ...
            if matches:
                issues.append(
                    f"Attribute '{attr}' is checked with hasattr - initialization to None might change behavior")
                ...
                for i, line in enumerate(lines):
                    if f'self.{attr}' in line:
                        ...
                        if 'except AttributeError' in context or 'except:' in context:
                            issues.append(
                                f"Attribute '{attr}' might be used in try/except block - needs manual review")
    ...
    try:
        ...
        for attr in modified_attrs:
            if attr in visitor.conditional_attrs:
                issues.append(
                    f"Attribute '{attr}' is created conditionally - initialization might affect logic")
