...
""")
    ...
    def run_pylint(self):
        ...
        try:
            ...
            if result.returncode != 0:
                self.log("Pylint not installed, skipping pylint analysis", "WARNING")
                return
            ...
            for line in lines:
                ...
                match = re.match(r'([^:]+):(\d+):\s*\[([A-Z]\d{4})\]\s*(.+)', line)
                if match:
                    file_path, line_no, code, msg = match.groups()
                    file_rel = file_path.replace('intellicrack/', '') if 'intellicrack/' in file_path else file_path
                    ...
                    self.add_issue(category, file_rel, int(line_no), f"[{code}] {msg}")
    ...
    def analyze_ast_patterns(self):
        ...
        self.log(f"Analyzed {patterns_found} files for AST patterns", "SUCCESS")
    ...
    def detect_import_issues(self):
        ...
        for root, dirs, files in os.walk(self.project_path):
            ...
            for file in files:
                ...
                filepath = Path(root) / file
                module_path = str(filepath.relative_to(self.project_path.parent)).replace(os.sep, '.')[:-3]
        ...
        self.log(f"Found {len(cycles)} import cycles", "WARNING" if cycles else "SUCCESS")
    ...
    def test_runtime_safety(self):
        ...
        try:
            ...
            self.log(f"Runtime safety testing failed: {type(e).__name__}: {e}", "ERROR")
    ...
    def check_security_issues(self):
        ...
        security_patterns = [
            ...
            (r'subprocess\.call\s*\([^)]*shell\s*=\s*True', 'SHELL_INJECTION', 'subprocess with shell=True'),
            (r'pickle\.loads?\s*\(', 'PICKLE_USAGE', 'Pickle usage (potential security risk)'),
            (r'yaml\.load\s*\([^)]*\)', 'UNSAFE_YAML', 'yaml.load without Loader (use safe_load)'),
    ...
    def generate_summary(self):
        ...
        with open(self.output_file, 'a', encoding='utf-8') as f:
            ...
            total_errors = sum(self.issue_counts[cat] for cat in error_categories)
            total_warnings = sum(self.issue_counts[cat] for cat in warning_categories) 
            total_security = sum(self.issue_counts[cat] for cat in security_categories)
            total_quality = sum(self.issue_counts[cat] for cat in quality_categories)
        ...
        # Console summary
        self.log("\n" + "="*60, "HEADER")
        self.log(f"ANALYSIS COMPLETE: {len(self.all_issues)} total issues found", "HEADER")
        self.log(f"Errors: {total_errors} | Warnings: {total_warnings} | Security: {total_security} | Quality: {total_quality}", "INFO")
