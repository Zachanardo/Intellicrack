...
class AIBinaryBridge:
    ...
    def _format_strings_for_prompt(self, strings: list[dict[str, Any]]) -> str:
        ...
        result = "".join(f"- Offset {s['offset']}: '{s['value']}' ({s['encoding']}, {s['size']} bytes)\n" for s in strings[:10])
        if len(strings) > 10:
    ...
    def _calculate_analysis_confidence(self, patterns: list[dict], anomalies: list[dict], suspicious_strings: list[str]) -> float:
        ...
        if patterns:
            ...
            if protection_patterns := [p for p in patterns if p.get("pattern_type") in ["license_check", "anti_debug", "encryption"]]:
    ...
    def _calculate_search_confidence(self, matches: list[dict], query: str) -> float:
        ...
        if high_relevance := [m for m in matches if m.get("relevance_score", 0) > 0.8]:
