...
class ModelHealth:
    ...
    def get_success_rate(self, window_hours: int = 24) -> float:
        ...
        recent_failures = len([f for f in self.recent_failures if f.timestamp > cutoff])
...
class FallbackChain:
    ...
    def __init__(
        ...
        # Health tracking
        self.health_stats = {model_id: ModelHealth(model_id) for model_id, _ in model_configs}
    ...
    def _register_models(self) -> None:
        ...
        for model_id, config in self.model_configs:
            try:
                ...
                logger.debug("Registered model %s for chain %s", model_id, self.chain_id)
        ...
        with self.lock:
            ...
            if success:
                ...
                if health.is_circuit_open:
                    ...
                    logger.info("Circuit breaker closed for model %s", model_id)
    ...
    def _get_ordered_models(self) -> list[tuple[str, LLMConfig]]:
        ...
        def model_score(item: tuple[str, Any]) -> float:
            ...
            response_time_score = 1.0 / (1.0 + health.avg_response_time)  # Lower is better

            return success_rate * 0.7 + response_time_score * 0.3

        sorted_models = sorted(self.model_configs, key=model_score, reverse=True)
    ...
    async def chat_async(self, messages: list[LLMMessage], tools: list[dict[str, Any]] | None = None) -> LLMResponse | None:
        ...
        if not ordered_models:
            logger.error("No available models in chain %s", self.chain_id)
            return None
        ...
        for model_id, _config in ordered_models:
            ...
            if health.is_circuit_open and not health.should_retry():
                logger.debug("Skipping model %s - circuit breaker open", model_id)
                continue
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if response:
                        ...
                        logger.debug("Chain %s: Success with model %s (attempt %d)", self.chain_id, model_id, attempt + 1)
                        return response
                    ...
                    last_error = e
                    response_time = time.time() - start_time
                    self._update_health_stats(model_id, False, response_time, e)

                    failure_type = self._classify_error(e)

                    # Don't retry on permanent failures
                    if failure_type == FailureType.PERMANENT:
                        logger.warning("Permanent failure for model %s: %s", model_id, e)
                        break

                    # Calculate backoff delay
                    if attempt < self.max_retries - 1:
                        delay = self.retry_delay * (2**attempt) + random.uniform(0, 1)  # noqa: S311
                        logger.debug("Retrying model %s in %.2fs (attempt %d)", model_id, delay, attempt + 1)
                        await asyncio.sleep(delay)
                    else:
                        logger.warning("All retries exhausted for model %s: %s", model_id, e)

        logger.error("All models failed in chain %s. Last error: %s", self.chain_id, last_error)
        return None
    ...
    def get_health_report(self) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            for model_id, health in self.health_stats.items():
                ...
                models_dict = report["models"]
                assert isinstance(models_dict, dict)
                models_dict[model_id] = model_report
        ...
        with self.lock:
            if model_id:
                if model_id in self.health_stats:
                    ...
                    logger.info("Reset health stats for model %s", model_id)
            else:
                for mid in self.health_stats:
                    self.health_stats[mid] = ModelHealth(mid)
                logger.info("Reset health stats for all models in chain %s", self.chain_id)
...
class FallbackManager:
    ...
    def create_chain(
        ...
        with self.lock:
            ...
            chain = FallbackChain(
                ...
                enable_adaptive_ordering=enable_adaptive_ordering,
            )
            ...
            logger.info("Created fallback chain: %s", chain_id)
            return chain
    ...
    def set_default_chain(self, chain_id: str) -> None:
        ...
        self.default_chain_id = chain_id
        logger.info("Set default chain to: %s", chain_id)
    ...
    def chat(
        ...
        if not chain:
            logger.error("Chain not found: %s", target_chain_id)
            return None
    ...
    def get_global_health_report(self) -> dict[str, Any]:
        ...
        for chain_id, chain in self.chains.items():
            chains_dict = report["chains"]
            assert isinstance(chains_dict, dict)
            chains_dict[chain_id] = chain.get_health_report()
    ...
    def remove_chain(self, chain_id: str) -> bool:
        ...
        with self.lock:
            ...
            if self.default_chain_id == chain_id:
                self.default_chain_id = next(iter(self.chains.keys())) if self.chains else None

            logger.info("Removed fallback chain: %s", chain_id)
            return True
    ...
    def create_chain_from_config(self, config: dict[str, Any]) -> FallbackChain:
        ...
        for model_config in config["models"]:
            ...
            model_id = model_config.get("model_id", f"{chain_id}_{len(model_configs)}")
        ...
        assert isinstance(circuit_failure_threshold, int)
        assert isinstance(enable_adaptive_ordering, bool)
    ...
    def export_configuration(self) -> dict[str, Any]:
        ...
        for chain_id, chain in self.chains.items():
            ...
            for model_id, llm_config in chain.model_configs:
                ...
                models_list = chain_config["models"]
                assert isinstance(models_list, list)
                models_list.append(model_config)

            chains_dict = config["chains"]
            assert isinstance(chains_dict, dict)
            chains_dict[chain_id] = chain_config
...
def create_simple_fallback_chain(chain_id: str, model_ids: list[str], use_existing_configs: bool = True) -> FallbackChain:
    ...
    for model_id in model_ids:
        if use_existing_configs:
            ...
            if not model_info:
                logger.warning("Model %s not found in LLM manager", model_id)
                continue

            if actual_config := llm_manager.configs.get(model_id):
                model_configs.append((model_id, actual_config))
            else:
                logger.warning("Config for model %s not found in LLM manager configs", model_id)
        else:
            logger.warning("Model %s not found in LLM manager and use_existing_configs=False not supported", model_id)
            continue
