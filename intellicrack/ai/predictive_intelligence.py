            ...
            if fuzzing_dir.exists():
                for result_file in fuzzing_dir.glob("*.json"):
                    try:
                        ...
                        exploitable_crashes = sum(bool(c.get("exploitable", False)) for c in crashes)
...
class PredictiveIntelligenceEngine:
    ...
    def get_prediction_insights(self) -> dict[str, Any]:
        ...
        if len(self.prediction_history) > 10:
            ...
            if success_predictions := [p for p in recent_predictions if p.prediction_type == PredictionType.SUCCESS_PROBABILITY]:
            ...
            if time_predictions := [p for p in recent_predictions if p.prediction_type == PredictionType.EXECUTION_TIME]:
