...
try:
    ...
    from .llm_backends import LLMManager, get_llm_manager, LLMMessage, LLMResponse
        ...
        if subscribers:
            ...
            for subscriber in subscribers:
                try:
                    ...
                    def call_subscriber():
                        try:
                            subscriber["callback"](data, source_component)
                        except Exception as e:
                            logger.error(f"Error in subscriber {subscriber['component']}: {e}")
                    
                    threading.Thread(target=call_subscriber, daemon=True).start()
