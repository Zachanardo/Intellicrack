...
class AIAssistant:
        ...
        try:
            ...
            if not llm_manager:
                try:
                    ...
                    if llm_manager.get_available_llms():
    ...
    def _handle_llm_manager_question(self, question: str) -> str:
        if self._llm_manager is not None and hasattr(self._llm_manager, "chat"):
            ...
            response = self._llm_manager.chat(messages)
            if response and hasattr(response, "content"):
                return response.content or f"AI response to: {question}"
...
class CodeAnalyzer:
    ...
    def analyze_binary(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            findings = format_analysis.get("findings", [])
            ...
            if not result.get("confidence"):
                findings_len = len(result["findings"]) if isinstance(result.get("findings"), list) else 0  # type: ignore[arg-type]
                security_len = len(result["security_issues"]) if isinstance(result.get("security_issues"), list) else 0  # type: ignore[arg-type]
                result["confidence"] = min(
                    0.9,
                    0.5 + (findings_len * 0.1) + (security_len * 0.15),
                )
    ...
    def analyze_assembly(self, assembly_code: str) -> dict[str, Any]:
        ...
        try:
            ...
            patterns_count = len(result["patterns"]) if isinstance(result["patterns"], list) else 0
            vulns_count = len(result["vulnerabilities"]) if isinstance(result["vulnerabilities"], list) else 0
            result["confidence"] = min(0.95, 0.6 + (patterns_count * 0.08) + (vulns_count * 0.12))
    ...
    def _analyze_pe_format(self, file_path: str, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            if PEFILE_AVAILABLE and pefile is not None:
    ...
    def _analyze_cross_platform_format(self, file_path: str, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            if LIEF_AVAILABLE and lief is not None:
                if binary := lief.parse(file_path):
