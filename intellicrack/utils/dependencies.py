...
def check_weasyprint_dependencies() -> list[str]:
    ...
    if sys.platform == "win32":
        try:
                ...
                if os.path.exists(gtk_path):
                    for dll in ["libcairo-2.dll", "libgdk_pixbuf-2.0-0.dll", "libpango-1.0-0.dll"]:
                        ...
                        if os.path.exists(dll_path):
                            logger.info("OK Found GTK DLL: %s", dll_path)
                            dll_found = True
                            break
                    if dll_found:
                        break
            ...
            if not dll_found:
                logger.warning("FAIL GTK runtime libraries not found")
...
def check_and_install_dependencies() -> bool:
    ...
    for dep in core_deps:
        try:
            __import__(dep)
            logger.info("OK %s available", dep)
        except ImportError:
            logger.warning("FAIL %s missing", dep)
