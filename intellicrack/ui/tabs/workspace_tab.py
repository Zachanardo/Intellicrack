...
class WorkspaceTab(QWidget):
    ...
    def log_activity(self, message: str, level: str = "INFO") -> None:
        ...
        Args:
            message: The message text to log to the activity log display.
            level: The log level (INFO, WARNING, ERROR, SUCCESS). Defaults to
                INFO.
    ...
    def find_or_create_folder(self, folder_path: str) -> QTreeWidgetItem:
        ...
        Searches the tree for an existing folder item matching the given path,
        creating intermediate folder items as needed. Raises RuntimeError if
        the folder structure cannot be created.

        Args:
            folder_path: The folder path to find or create in the tree
                structure.
        ...
        Raises:
            RuntimeError: If the folder item cannot be created for the given
                path.
    ...
    def on_ai_message_sent(self, message: str) -> None:
        ...
        Logs the AI query to the activity log for history tracking. Truncates
        long messages to first 100 characters for display.

        Args:
            message: The message sent to the AI assistant.

        """
        display_msg = message[:100] + "..." if len(message) > 100 else message
        self.log_activity(f"AI Query: {display_msg}", "INFO")

    def on_code_generated(self, code: str) -> None:
        """Handle code generation from AI.

        Displays the generated code in the activity log and appends it to
        activity text for easy access and history tracking.

        Args:
            code: The code snippet generated by the AI assistant.
        ...
        if code and code.strip():
            ...
            self.activity_log_text.append(code)
            self.activity_log_text.append("--- End of Code ---\n")
    ...
    def on_script_generated(self, script_type: str, content: str) -> None:
        ...
        Logs the script generation event and optionally saves the script to
        the project scripts folder for later use or modification.

        Args:
            script_type: The type of script generated (e.g., "Frida",
                "Ghidra").
            content: The content of the generated script.
