        ...
        try:
            ...
            if results and isinstance(results, dict):
                ...
                if strings := results.get("extracted_strings"):
                    license_strings = [s for s in strings if any(kw in s.lower() for kw in ["license", "serial", "key", "trial", "expire"])]
                    if license_strings:
                ...
                if taint_paths := results.get("taint_paths"):
                    ...
                    for path in taint_paths[:15]:
                        if isinstance(path, dict):
                            ...
                            self.tool_output.append(f"  {source} -> {sink} (confidence: {confidence:.1%}, length: {length})")
                ...
                if validation_points := results.get("validation_points"):
                    ...
                    for vp in validation_points[:10]:
                        if isinstance(vp, dict):
                            ...
                            self.tool_output.append(f"  [{vp_type}] @ {vp_addr} (bypass difficulty: {difficulty})")
            ...
            if chains:
                ...
                for chain in chains[:5]:
                    if isinstance(chain, dict):
                        ...
                        self.tool_output.append(f"  [{chain_type}] Target: {target}")
                        self.tool_output.append(f"    Gadgets: {gadget_count}, Success Probability: {probability:.1%}")
                ...
                if protections := results.get("detected_protections"):
                    ...
                    for prot in protections[:10]:
                        if isinstance(prot, dict):
                            ...
                            self.tool_output.append(f"  [{prot_type}] @ {prot_addr} (confidence: {confidence:.1%})")
                ...
                if bypasses := results.get("generated_bypasses"):
                    ...
                    for bypass in bypasses[:5]:
                        if isinstance(bypass, dict):
                            ...
                            self.tool_output.append(f"  [{bypass_type}] Target: {target}, Size: {payload_size} bytes{stealth_str}")
                ...
                if patches:
                    ...
                    for patch in patches[:5]:
                        if isinstance(patch, dict):
                            ...
                            self.tool_output.append(
                                f"  [{patch_type}] Offset: 0x{offset:08x}, Original: {orig_size}B -> Patched: {new_size}B"
                            )
            else:
    ...
    def run_shellcode_generator(self) -> None:
        ...
        try:
            ...
            if results and isinstance(results, dict):
                ...
                total_bytes = sum(len(sc.get("code", b"")) for sc in shellcodes if isinstance(sc.get("code"), (bytes, bytearray)))
