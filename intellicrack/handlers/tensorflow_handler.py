...
try:
    ...
    logger.error("TensorFlow not available, using fallback implementations: %s", e)
    ...
    class FallbackModel:
        ...
        def summary(self):
            ...
            for i, layer in enumerate(self.layers):
                logger.info("  Layer %d: %s", i + 1, layer.name if hasattr(layer, 'name') else type(layer).__name__)
...
__all__ = [
    # Availability flags
    "HAS_TENSORFLOW", "TENSORFLOW_VERSION",
    # Main modules
    "tf", "tensorflow", "keras",
    # Submodules
