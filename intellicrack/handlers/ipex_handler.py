...
def _setup_ipex_dll_paths():
    ...
    try:
        ...
        for base_path in oneapi_base_paths:
            if not base_path.exists():
                continue

            mkl_paths = [
                base_path / "2025.2" / "bin",
                base_path / "latest" / "bin",
                base_path / "mkl" / "2025.2" / "bin",
                base_path / "mkl" / "latest" / "bin",
            ]
            for mkl_path in mkl_paths:
                if mkl_path.exists() and any((mkl_path / f"mkl_{x}.dll").exists() for x in ["core.2", "sycl_blas.5"]):
                    dll_dirs.append(mkl_path)
                    logger.debug("Found MKL at: %s", mkl_path)
                    break

            umf_paths = [
                base_path / "2025.2" / "bin",
                base_path / "latest" / "bin",
                base_path / "umf" / "0.11" / "bin",
            ]
            for umf_path in umf_paths:
                if umf_path.exists() and (umf_path / "umf.dll").exists():
                    dll_dirs.append(umf_path)
                    logger.debug("Found UMF (Unified Memory Framework) at: %s", umf_path)
                    break

            compiler_paths = [
                base_path / "compiler" / "2025.2" / "bin",
                base_path / "compiler" / "latest" / "bin",
            ]
            for compiler_path in compiler_paths:
                if compiler_path.exists():
                    dll_dirs.append(compiler_path)
                    logger.debug("Found Intel oneAPI compiler runtime at: %s", compiler_path)
                    break

            tbb_paths = [
                base_path / "tbb" / "2022.2" / "bin",
                base_path / "tbb" / "latest" / "bin",
            ]
            for tbb_path in tbb_paths:
                if tbb_path.exists():
                    dll_dirs.append(tbb_path)
                    logger.debug("Found TBB at: %s", tbb_path)
                    break
        ...
        for lz_base in level_zero_paths:
            if lz_base.exists():
                for lz_dir in lz_base.glob("iigd_dch.inf_amd64_*"):
                    if (lz_dir / "ze_loader.dll").exists():
                        dll_dirs.append(lz_dir)
                        logger.debug("Found Level Zero runtime at: %s", lz_dir)
                        break
