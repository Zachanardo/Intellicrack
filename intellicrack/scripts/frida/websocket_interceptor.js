...
const credentialGenerator = {
    ...
    learnCredentialPattern(credentialType, observedValue) {
    ...
    extractPrefix: value => {
        const prefixMatch = value.match(/^([A-Z_a-z]{2,8})[\dA-Za-z]/);
        return prefixMatch ? prefixMatch[1] : '';
    },

    // Extract common suffixes
    extractSuffix: value => {
        const suffixMatch = value.match(/[\dA-Za-z]([A-Z_a-z]{2,8})$/);
    ...
    generateCredential(credentialType, targetLength = null, targetPrefix = null) {
    ...
    generateFromPattern(pattern, overrideLength = null, overridePrefix = null) {
    ...
    generateBase64: targetLength => {
        ...
        return btoa(result).slice(0, Math.max(0, targetLength));
    },
    ...
    generateDefault(credentialType, targetLength = null, targetPrefix = null) {
...
const websocketInterceptor = {
    ...
    hookWebSocketConstructor() {
        ...
        try {
            ...
            send({
                type: 'debug',
                message: `Browser WebSocket hook failed: ${error.message}`,
            });
        }
    ...
    shouldInterceptUrl(url) {
        ...
        for (let i = 0; i < this.config.targetUrls.length; i++) {
            const pattern = this.config.targetUrls[i].replaceAll('*', '.*');
    ...
    // Spoofing handlers
    spoofVerifyResponse(originalMessage) {
        try {
    ...
    spoofLicenseResponse(originalMessage) {
        try {
            const parsed = JSON.parse(originalMessage);
    ...
    spoofValidateResponse(originalMessage) {
        try {
            const parsed = JSON.parse(originalMessage);
    ...
    spoofAuthResponse(originalMessage) {
        try {
            const parsed = JSON.parse(originalMessage);
            ...
            Interceptor.attach(winHttpReceiveResponse, {
                onLeave(retval) {
                    if (retval.toInt32() !== 0) {
                        ...
                        const request = this.context.rcx;
                ...
                Interceptor.attach(wsCreateClientHandle, {
                    onEnter: args => {
                        ...
                        if (!subProtocols.isNull()) {
                            send(
                                `[WebSocket] Client handle with subProtocols: ${subProtocols.readUtf8String()}`
                            );
    ...
    processProtobufMessage: message => {
        ...
        if (
            ...
            let modified = message;
            modified = modified.replaceAll(/valid[\s"]*:[\s"]*false/gi, 'valid": true');
            modified = modified.replaceAll(/expired[\s"]*:[\s"]*true/gi, 'expired": false');
            modified = modified.replaceAll(/trial[\s"]*:[\s"]*true/gi, 'trial": false');
                    ...
                    onEnter: args => {
                        ...
                        try {
                            ...
                            send({
                                ...
                                error_details: {
                                    error_type: error.name || 'MsgPackError',
                                    error_message: error.message || error.toString(),
    ...
    setupAuthenticationBypass() {
        ...
        try {
            ...
            send({
                ...
                error: error.toString(),
                error_analysis: {
                    error_type: error.name || 'AuthBypassError',
                    error_details: error.message || error.toString(),
                    attempted_bypasses: {
        ...
        try {
            ...
            for (const module in imports) {
                if (Object.hasOwn(imports, module)) {
                    ...
                    for (const func in moduleImports) {
                    ...
                    onEnter: args => {
                        ...
                        if (
                            ...
                            if (option === WINHTTP_OPTION_SECURITY_FLAGS) {
                                ...
                                flags.writeU32(0x33_00); // SECURITY_FLAG_IGNORE_ALL_CERT_ERRORS
                                args[2] = flags;
                            }
                        }
                    },
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        send({
                                            ...
                                            headers: responseHeaders.slice(0, 500),
                                        });
    ...
    handleLicenseProtocol: message => {
        ...
        modified = modified.replaceAll(/status.*invalid/gi, 'status: valid');
        modified = modified.replaceAll(/licensed.*false/gi, 'licensed: true');
        modified = modified.replaceAll(/expired.*true/gi, 'expired: false');

        return modified;
    },

    // Handle default protocol messages
    handleDefaultProtocol(message) {
    ...
    processDnsResponse: (buffer, length) => {
        try {
            ...
            const header = buffer.readByteArray(12);
...
if (websocketInterceptor !== undefined) {
    send({
        type: 'info',
