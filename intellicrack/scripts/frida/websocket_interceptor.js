...
const websocketInterceptor = {
    ...
    hookWebRTCDataChannels: function() {
        ...
        try {
            ...
            if (typeof RTCPeerConnection !== 'undefined') {
                ...
                RTCPeerConnection = function(config) {
                    if (self.config.webRtcConfig.spoofIceServers && config && config.iceServers) {
                        config.iceServers = self.config.webRtcConfig.overrideStunServers.map(url => ({
                            urls: url
                        }));
                        send({
                            type: 'bypass',
            ...
            if (webRtcModule) {
                var createDataChannel = Module.findExportByName(webRtcModule.name, 'CreateDataChannel');
    ...
    hookHTTP3QuicConnections: function() {
        ...
        try {
            ...
            if (quicModule) {
                var quicConnectionOpen = Module.findExportByName('msquic.dll', 'MsQuicConnectionOpen');
                if (quicConnectionOpen) {
                    Interceptor.attach(quicConnectionOpen, {
                        ...
                        onLeave: function(retval) {
                            if (retval.toInt32() === 0) { // QUIC_STATUS_SUCCESS
                ...
                var quicStreamReceive = Module.findExportByName('msquic.dll', 'MsQuicStreamReceiveSetEnabled');
    ...
    hookWebSocketExtensions: function() {
        ...
        try {
            ...
            var wsCreateClientHandle = Module.findExportByName('websocket.dll', 'WebSocketCreateClientHandle');
                ...
                Interceptor.attach(wsCreateClientHandle, {
                    onEnter: function(args) {
                        ...
                        if (!extensions.isNull()) {
                            ...
                            var extensionList = self.config.wsExtensions.supportedExtensions.join(';');
    ...
    hookProtobufDecoding: function() {
        ...
        protobufModules.forEach(function(moduleName) {
            ...
            var parseFromString = Module.findExportByName(moduleName, '_ZN6google8protobuf7Message15ParseFromStringERKSs');
            if (parseFromString) {
    ...
    processProtobufMessage: function(message) {
        ...
        if (message.includes('license') || message.includes('valid') || message.includes('expire')) {
            send({
                type: 'info',
                target: 'websocket_interceptor',
    ...
    hookCapnProtoDecoding: function() {
        ...
        if (capnpModule) {
            ...
            var readMessage = Module.findExportByName('capnp.dll', '_ZN6capnp11MessageReader11readMessageERNS_11InputStreamEi');
            if (readMessage) {
                    ...
                    onEnter: function(args) {
                        ...
                        if (!data.isNull() && dataLen > 16) {
                            try {
                                ...
                                if (keyData.match(/^[A-Za-z0-9+\/=]{16,}$/) || keyData.includes('key') || keyData.includes('api')) {
                                    send({
                                        type: 'info',
                                        target: 'websocket_interceptor',
                ...
                EventSource = function(url, config) {
                    ...
                    if (self.shouldInterceptUrl(url)) {
                        ...
                        eventSource.addEventListener = function(type, listener, options) {
                            if (type === 'message' || type === 'error' || type.includes('license')) {
                        ...
                        var messageDescriptor = Object.getOwnPropertyDescriptor(EventSource.prototype, 'onmessage') ||
                ...
                if (winHttpOpenRequest) {
                    Interceptor.attach(winHttpOpenRequest, {
                        onEnter: function(args) {
                            ...
                            if (verb === 'GET' && (objectName.includes('/events') || objectName.includes('/stream'))) {
                    ...
                    Interceptor.attach(winHttpReceiveResponse, {
                        ...
                        onLeave: function(retval) {
                            if (retval.toInt32() !== 0) {
                                ...
                                var queryResult = Module.findExportByName('winhttp.dll', 'WinHttpQueryHeaders');
                                if (queryResult) {
                                    var WINHTTP_QUERY_CONTENT_TYPE = 1;
                                    var queryFunc = new NativeFunction(queryResult, 'int', ['pointer', 'uint32', 'pointer', 'pointer', 'pointer']);

                                    if (queryFunc(this.request, WINHTTP_QUERY_CONTENT_TYPE, NULL, buffer, sizeNeeded) !== 0 ||
                                        Process.getLastError() === 122) { // ERROR_INSUFFICIENT_BUFFER
            ...
            if (typeof WebTransport !== 'undefined') {
                ...
                WebTransport = function(url, options) {
                    ...
                    if (self.shouldInterceptUrl(url)) {
                        ...
                        transport.incomingUnidirectionalStreams.getReader().read().then(function processStream(result) {
                            if (!result.done) {
                        ...
                        transport.incomingBidirectionalStreams.getReader().read().then(function processStream(result) {
                            if (!result.done) {
                                var stream = result.value;
                                send({
                                    type: 'info',
                        ...
                        transport.datagrams.readable.getReader().read().then(function processDatagram(result) {
                            if (!result.done) {
    ...
    hookWebTransportStream: function(stream, type) {
        ...
        try {
            ...
            if (stream.readable) {
                stream.readable.getReader().read().then(function processData(result) {
                    if (!result.done) {
                        var data = result.value;
                        var dataStr = new TextDecoder().decode(data);
    ...
    hookGrpcWeb: function() {
        ...
        try {
            ...
            if (typeof grpc !== 'undefined' && grpc.web) {
                ...
                grpc.web.AbstractClientBase.prototype.rpcCall = function(method, request, metadata, methodDescriptor, callback) {
                    send({
                        type: 'info',
                        target: 'websocket_interceptor',
                    ...
                    if (method.includes('license') || method.includes('validate') || method.includes('auth')) {
                        send({
                            type: 'info',
                            target: 'websocket_interceptor',
                    ...
                    return originalCall.call(this, method, request, metadata, methodDescriptor, callback);
                };
            }
    ...
    processGrpcOperations: function(ops, count) {
        try {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (opType === 0 || opType === 1) { // GRPC_OP_SEND_MESSAGE or GRPC_OP_RECV_MESSAGE
                    ...
                    onLeave: function(retval) {
                        ...
                        if (result === 1) { // BROTLI_DECODER_RESULT_SUCCESS
                            ...
                            if (!this.nextOut.isNull() && outputSize > 0) {
                                try {
                                    var decompressedData = this.nextOut.readUtf8String(Math.min(outputSize, 1024));
                ...
                Interceptor.attach(compress, {
                    onEnter: function(args) {
                        ...
                        if (!this.nextIn.isNull() && this.availableIn.readU32() > 0) {
                            try {
                                var inputData = this.nextIn.readUtf8String(Math.min(this.availableIn.readU32(), 1024));
                ...
                Interceptor.attach(decompress, {
                    ...
                    onLeave: function(retval) {
                        ...
                        if (decompressedSize > 0) {
                            ...
                            if (!this.dest.isNull()) {
                                try {
                                    var decompressedData = this.dest.readUtf8String(Math.min(decompressedSize, 1024));
                                    var modified = self.processIncomingMessage(decompressedData);
                ...
                Interceptor.attach(compress, {
                    onEnter: function(args) {
                        ...
                        if (!this.source.isNull() && this.sourceSize > 0) {
                            try {
                                var inputData = this.source.readUtf8String(Math.min(this.sourceSize, 1024));
                                var modified = self.processOutgoingMessage(inputData);
                ...
                Interceptor.attach(decompress, {
                    ...
                    onLeave: function(retval) {
                        ...
                        if (decompressedSize > 0 && !this.dst.isNull()) {
                            ...
                            try {
                                var decompressedData = this.dst.readUtf8String(Math.min(decompressedSize, 1024));
                                var modified = self.processIncomingMessage(decompressedData);
                ...
                Interceptor.attach(compress, {
                    onEnter: function(args) {
                        ...
                        if (!this.src.isNull() && this.srcSize > 0) {
                            try {
                                var inputData = this.src.readUtf8String(Math.min(this.srcSize, 1024));
                                var modified = self.processOutgoingMessage(inputData);
    ...
    hookWebhookHttpServer: function() {
        ...
        var winHttpModule = Process.findModuleByName('winhttp.dll');
        if (winHttpModule) {
            var winHttpReceiveRequest = Module.findExportByName('winhttp.dll', 'WinHttpReceiveRequest');
                ...
                Interceptor.attach(winHttpReceiveRequest, {
                    ...
                    onLeave: function(retval) {
                        if (retval.toInt32() !== 0) {
                            ...
                            var queryHeaders = Module.findExportByName('winhttp.dll', 'WinHttpQueryHeaders');
                            if (queryHeaders) {
                                var WINHTTP_QUERY_REQUEST_METHOD = 45;
                                var queryFunc = new NativeFunction(queryHeaders, 'int', ['pointer', 'uint32', 'pointer', 'pointer', 'pointer']);

                                if (queryFunc(this.request, WINHTTP_QUERY_REQUEST_METHOD, NULL, buffer, sizeNeeded) !== 0) {
                                    ...
                                    if (method === 'POST' || method === 'PUT') {
                                        ...
                                        if (queryFunc(this.request, WINHTTP_QUERY_RAW_HEADERS_CRLF, NULL, urlBuffer, urlSize) !== 0) {
                                            var headers = urlBuffer.readUtf8String(urlSize.readU32());

                                            // Look for license-related webhook endpoints
                                            if (headers.includes('license') || headers.includes('webhook') ||
                                                headers.includes('validate') || headers.includes('callback')) {
                    ...
                    if (module === 'express' && result) {
                        ...
                        result.Router = function() {
                            ...
                            router.post = function(path) {
                                if (path.includes('webhook') || path.includes('license') || path.includes('callback')) {
                                    send({
                                        type: 'info',
                                        target: 'websocket_interceptor',
    ...
    processWebhookRequest: function(request, headers) {
        ...
        try {
            ...
            if (winHttpReadData) {
                ...
                var readFunc = new NativeFunction(winHttpReadData, 'int', ['pointer', 'pointer', 'uint32', 'pointer']);
        ...
        try {
            ...
            if (winHttpSendResponse) {
                ...
                var sendFunc = new NativeFunction(winHttpSendResponse, 'int',
                    ['pointer', 'pointer', 'uint32', 'pointer', 'uint32', 'pointer', 'uint32', 'pointer']);

                sendFunc(request, statusCode, statusCode.readUtf8String().length,
                    headers, headers.readUtf8String().length,
                    responseBuffer, response.length, NULL);
            ...
            if (typeof WebAssembly !== 'undefined') {
                ...
                if (originalInstantiateStreaming) {
                    WebAssembly.instantiateStreaming = function(source, imports) {
                        ...
                        return originalInstantiateStreaming.call(this, source, imports).then(function(result) {
                            if (result.instance && result.instance.exports) {
                                self.hookWasmExports(result.instance.exports);
                            }
                            return result;
                    ...
                    for (var func in moduleImports) {
                        if (moduleImports.hasOwnProperty(func)) {
                            ...
                            if (typeof originalFunc === 'function' &&
                                (func.includes('send') || func.includes('message') ||
                                 func.includes('validate') || func.includes('license') ||
                                 func.includes('check') || func.includes('auth'))) {
        ...
        try {
            for (var func in exports) {
                if (exports.hasOwnProperty(func) && typeof exports[func] === 'function') {
                    ...
                    if (func.includes('validate') || func.includes('license') ||
                        func.includes('check') || func.includes('auth') ||
                        func.includes('send') || func.includes('receive')) {
                    ...
                    if (func) {
                        Interceptor.attach(func, {
                            onEnter: function(args) {
                                ...
                                this.isLicenseFunction = funcName.includes('license') ||
            ...
            handshakeFunctions.forEach(function(funcName) {
                ...
                if (func) {
                    Interceptor.attach(func, {
                        ...
                        onLeave: function(retval) {
                            var result = retval.toInt32();
                            if (result === 0 && funcName === 'SslDecryptPacket' && this.bufferSize > 0) {
                                // Process decrypted TLS data
                                try {
                                    var decryptedData = this.buffer.readUtf8String(Math.min(this.bufferSize, 1024));
                                    if (decryptedData.includes('license') || decryptedData.includes('validate')) {
                                        send({
                                            type: 'info',
                                            target: 'websocket_interceptor',
                    ...
                    onLeave: function(retval) {
                        ...
                        if (bytesRead > 0 && !this.buf.isNull()) {
                            try {
                                ...
                                if (data.includes('license') || data.includes('validate') || data.includes('auth')) {
                                    send({
                                        type: 'info',
                                        target: 'websocket_interceptor',
            ...
            if (winHttpSetOption) {
                Interceptor.attach(winHttpSetOption, {
                    onEnter: function(args) {
                        ...
                        if (option === WINHTTP_OPTION_SERVER_CERT_CONTEXT || option === WINHTTP_OPTION_SECURITY_FLAGS) {
                            send({
                                type: 'bypass',
                                target: 'websocket_interceptor',
                ...
                Interceptor.attach(winHttpConnect, {
                    onEnter: function(args) {
                        ...
                        if (!serverName.isNull()) {
                            ...
                            if (originalName.includes('license') || originalName.includes('activate') ||
                                originalName.includes('validate') || originalName.includes('auth')) {
        ...
        ws.send = function(data) {
            try {
                ...
                if (message.type === 'start' && message.payload) {
                    ...
                    if (query.includes('license') || query.includes('validate') ||
                        query.includes('subscription') || Object.keys(variables).some(k =>
                        k.includes('license') || k.includes('auth'))) {
        ...
        try {
            if (typeof require !== 'undefined') {
                ...
                require = function(module) {
                    ...
                    if ((module === 'graphql' || module === 'apollo-server' ||
                         module === 'apollo-client') && result) {
                        send({
                            type: 'info',
                            target: 'websocket_interceptor',
                                        ...
                                        if (def.selectionSet && def.selectionSet.selections) {
                                            def.selectionSet.selections.forEach(function(selection) {
                                                if (selection.name && (
                                                    ...
                                                    args.variableValues = self.modifyGraphQLVariables(variables);
                                                }
                                            });
                                        }
                                    });
                ...
                Interceptor.attach(winHttpSendRequest, {
                    onEnter: function(args) {
                        ...
                        if (!headers.isNull() && headersLength > 0) {
                            ...
                            if (headerString.includes('Upgrade: websocket') &&
                                ...
                                var protocolMatch = headerString.match(/Sec-WebSocket-Protocol:\s*([^\r\n]+)/i);
                                if (protocolMatch) {
                                    var protocols = protocolMatch[1].split(',').map(p => p.trim());
                                    send({
                                        type: 'info',
                                        target: 'websocket_interceptor',
                    ...
                    onLeave: function(retval) {
                        if (retval.toInt32() !== 0) {
                            ...
                            if (queryHeaders) {
                                ...
                                if (queryFunc(request, WINHTTP_QUERY_RAW_HEADERS_CRLF, NULL, buffer, sizeNeeded) !== 0) {
                                    ...
                                    if (responseHeaders.includes('101 Switching Protocols') &&
                                        ...
                                        var protocolMatch = responseHeaders.match(/Sec-WebSocket-Protocol:\s*([^\r\n]+)/i);
                                        if (protocolMatch) {
    ...
    hookCustomProtocolFormats: function() {
        ...
        var customFormats = [
            { name: 'base64-json', detector: /^[A-Za-z0-9+\/]+=*$/, decoder: self.decodeBase64Json },
            { name: 'hex-encoded', detector: /^[0-9A-Fa-f]+$/, decoder: self.decodeHexData },
            { name: 'custom-header', detector: /^LICE[0-9A-F]{4}/, decoder: self.decodeCustomHeader }
        ];
        ...
        if (winSocketModule) {
            ...
            if (connect) {
                Interceptor.attach(connect, {
                    ...
                    onLeave: function(retval) {
                        if (this.isDotConnection && retval.toInt32() === 0) { // SOCKET_ERROR = -1, success = 0
                            send({
                                type: 'info',
                                target: 'websocket_interceptor',
                ...
                Interceptor.attach(getAddrInfoEx, {
                    onEnter: function(args) {
                        ...
                        if (!nodeName.isNull()) {
                            ...
                            if (hostname.includes('license') || hostname.includes('activate') ||
                                hostname.includes('validate') || hostname.includes('auth') ||
    ...
    processDnsQuery: function(buffer, length) {
        try {
            ...
            if (domainName && (domainName.includes('license') ||
    ...
    processDnsResponse: function(buffer, length) {
        try {
            ...
            if (isResponse && responseCode === 0) { // NOERROR
