...
const NtpBlocker = {
  ...
  blockVirtualizationTimeSync: function () {
    ...
    if (qgaTimeSync) {
      Interceptor.replace(
        qgaTimeSync,
        new NativeCallback(
          function (time_ns, has_time, errp) {
            send({
              type: "bypass",
              target: "ntp_blocker",
  ...
  blockSecureEnclaveTime: function () {
    ...
    if (sgx_get_trusted_time) {
      Interceptor.replace(
        sgx_get_trusted_time,
        new NativeCallback(
          function (current_time, time_source_nonce) {
            send({
              type: "bypass",
              target: "ntp_blocker",
  ...
  blockCloudMetadataTime: function () {
    ...
    var getaddrinfo = Module.findExportByName(null, "getaddrinfo");
    if (getaddrinfo) {
      var originalGetaddrinfo = new NativeFunction(getaddrinfo, "int", [
        "pointer",
        "pointer",
        "pointer",
        "pointer",
      ...
      Interceptor.replace(
        getaddrinfo,
        new NativeCallback(
          function (node, service, hints, res) {
  ...
  blockWebRTCTimeHeaders: function () {
    ...
    var sendto = Module.findExportByName(null, "sendto");
    if (sendto) {
      var originalSendto = new NativeFunction(sendto, "int", [
        "int",
        "pointer",
        "int",
        "int",
      ...
      Interceptor.replace(
        sendto,
        new NativeCallback(
          function (sockfd, buf, len, flags, dest_addr, addrlen) {
            if (buf && len >= 20) {
