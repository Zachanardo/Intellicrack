...
const NtpBlocker = {
            ...
            Interceptor.attach(recv, {
                ...
                onLeave: function (retval) {
                    if (retval.toInt32() >= 48 && this.buf) {
                      ...
                      const li_vn_mode = view[0];
                      const stratum = view[1];
                      const _poll = view[2];
                      const _precision = view[3];
                      ...
                      if (mode >= 3 && mode <= 5 && version >= 3 && version <= 4) {
                            // Extract timestamps
                          const refTimestamp = new Uint32Array(packet.slice(16, 24));
                          const origTimestamp = new Uint32Array(packet.slice(24, 32));
                          const recvTimestamp = new Uint32Array(packet.slice(32, 40));
                          const transmitTimestamp = new Uint32Array(packet.slice(40, 48));
    ...
    blockModernTimeSyncDaemons: function () {
      ...
      const dbus_message_new_method_call = Module.findExportByName(
        null,
    ...
    blockGPSTimeSync: function () {
      ...
      if (openat) {
        ...
        // Block GPSD daemon
      const connect_gpsd = Module.findExportByName(null, 'gps_open');
      if (connect_gpsd) {
            Interceptor.replace(
                connect_gpsd,
                new NativeCallback(
    ...
    blockHardwareClockAccess: function () {
      ...
      const clock_settime = Module.findExportByName(null, 'clock_settime');
      if (clock_settime) {
            Interceptor.replace(
                clock_settime,
                new NativeCallback(
        ...
        // Block settimeofday
      const settimeofday = Module.findExportByName(null, 'settimeofday');
      if (settimeofday) {
            Interceptor.replace(
                settimeofday,
                new NativeCallback(
      ...
      const adjtime = Module.findExportByName(null, 'adjtime');
      if (adjtime) {
            Interceptor.replace(
                adjtime,
                new NativeCallback(
      ...
      const adjtimex = Module.findExportByName(null, 'adjtimex');
      if (adjtimex) {
            Interceptor.replace(
                adjtimex,
                new NativeCallback(
    ...
    blockVirtualizationTimeSync: function () {
      ...
      if (setns) {
        ...
        if (Process.platform === 'windows') {
          const vmicsvc = Module.findExportByName('vmicres.dll', 'VmIcTimeSync');
          if (vmicsvc) {
                Interceptor.replace(
                    vmicsvc,
                    new NativeCallback(
      ...
      const qgaTimeSync = Module.findExportByName(null, 'qga_guest_set_time');
      if (qgaTimeSync) {
            Interceptor.replace(
                qgaTimeSync,
                new NativeCallback(
                    (_time_ns, _has_time, _errp) => {
                        send({
                            type: 'bypass',
                            target: 'ntp_blocker',
      ...
      // Android-specific time sync blocking
        if (Process.platform === 'linux' && Java.available) {
            Java.perform(() => {
              ...
              AlarmManager.setTime.implementation = millis => {
                    send({
                        type: 'bypass',
                        target: 'ntp_blocker',
                ...
                try {
                  const TimeManager = Java.use('android.app.time.TimeManager');
                  TimeManager.suggestExternalTime.implementation = _timeSuggestion => {
                        send({
                            type: 'bypass',
                            target: 'ntp_blocker',
        ...
        if (ObjC.available) {
          ...
          const NSTimeZone = ObjC.classes.NSTimeZone;
          if (NSTimeZone) {
    ...
    blockDistributedTimeProtocols: function () {
      ...
      const _rpcPorts = [111, 2049]; // Portmapper and NFS
      ...
      if (clnt_create) {
            Interceptor.attach(clnt_create, {
                onEnter: function (args) {
                  ...
                  const _proto = args[3].readUtf8String();
      ...
      if (getaddrinfo) {
          const _original_getaddrinfo = self.hookDNSResolution;
    ...
    preventTimeCorrelationAttacks: function () {
            ...
            Interceptor.attach(gettimeofday, {
                ...
                onLeave: function (retval) {
                    if (retval.toInt32() === 0 && this.tv) {
                      ...
                      let tv_usec = this.tv.add(8).readU64();
                ...
                onEnter: function (args) {
                  ...
                  // CLOCK_MONOTONIC = 1, CLOCK_MONOTONIC_RAW = 4
                    if ((clockid === 1 || clockid === 4) && timespec) {
                      ...
                      const base_monotonic = self._getBaseMonotonicTime();
                      const controlled_sec = base_monotonic.seconds;
                      const controlled_nsec = base_monotonic.nanoseconds;
      ...
      const send_socket = Module.findExportByName(null, 'send');
      if (send_socket) {
            ...
            Interceptor.attach(connect, {
                ...
                onLeave: function (retval) {
                    if (this.shouldBlock) {
                        ...
                        if (Process.platform === 'windows') {
                          const WSASetLastError = Module.findExportByName(
                            'ws2_32.dll',
                            'WSASetLastError'
                          );
                          if (WSASetLastError) {
    ...
    hookWindowsTimeService: function () {
      ...
      if (w32TimeSetConfig) {
        ...
        // W32TimeSyncNow
      const w32TimeSyncNow = Module.findExportByName('w32time.dll', 'W32TimeSyncNow');
      if (w32TimeSyncNow) {
            Interceptor.replace(
                w32TimeSyncNow,
                new NativeCallback(
      ...
      const startService = Module.findExportByName('advapi32.dll', 'StartServiceW');
      if (startService) {
    ...
    hookTimeRegistryAccess: function () {
      ...
      // RegSetValueEx
        ['RegSetValueExW', 'RegSetValueExA'].forEach(api => {
    ...
    isBlockedIP: function (ip) {
        ...
        for (let i = 0; i < this.config.timeServers.length; i++) {
    ...
    startMonitoring: function () {
        ...
        setInterval(() => {
            send({
                ...
                stats: {
                    total_blocked: this.stats.totalBlocked,
                    dns_blocked: this.stats.dnsBlocked,
                    connections_blocked: this.stats.connectionsBlocked,
                    http_blocked: this.stats.httpBlocked,
                    service_blocked: this.stats.serviceBlocked,
    ...
    // Monitor for new time server patterns
    monitorNewTimeServers: () => {
        // Hook system log functions to detect new time servers
        if (Process.platform === 'windows') {
            // OutputDebugString
          const outputDebugString = Module.findExportByName('kernel32.dll', 'OutputDebugStringW');
    ...
    blockTLSCertificateTimestamps: function () {
      ...
      if (X509_verify_cert) {
            Interceptor.attach(X509_verify_cert, {
                onEnter: args => {
                  ...
                  if (ctx) {
                        ...
                        // Flag value = 0x200000
                      const set_flags = Module.findExportByName(null, 'X509_STORE_CTX_set_flags');
                      if (set_flags) {
        ...
        if (Process.platform === 'windows') {
          const CertVerifyTimeValidity = Module.findExportByName(
            'crypt32.dll',
      ...
      const gnutls_x509_crt_verify = Module.findExportByName(null, 'gnutls_x509_crt_verify');
            ...
            Interceptor.attach(gnutls_x509_crt_verify, {
                onEnter: args => {
                  ...
                  if (flags) {
                      ...
                      const currentFlags = flags.readU32();
    ...
    blockTPMandHSMTime: function () {
      ...
      // Block TPM 2.0 time queries on Windows
        if (Process.platform === 'windows') {
          ...
          const Tbsi_Physical_Presence_Command = Module.findExportByName(
            'tbsapi.dll',
    ...
    blockSecureEnclaveTime: function () {
      ...
      const sgx_get_trusted_time = Module.findExportByName(null, 'sgx_get_trusted_time');
      if (sgx_get_trusted_time) {
            Interceptor.replace(
                sgx_get_trusted_time,
                new NativeCallback(
                    (_current_time, _time_source_nonce) => {
                        send({
                            type: 'bypass',
                            target: 'ntp_blocker',
      ...
      const sgx_create_pse_session = Module.findExportByName(null, 'sgx_create_pse_session');
      ...
      const TEE_GetSystemTime = Module.findExportByName(null, 'TEE_GetSystemTime');
    ...
    blockCloudMetadataTime: function () {
      ...
      const connect = Module.findExportByName(null, 'connect');
      if (connect) {
          const originalConnect = new NativeFunction(connect, 'int', ['int', 'pointer', 'int']);
          Interceptor.replace(
                connect,
                new NativeCallback(
                    (sockfd, addr, addrlen) => {
      ...
      const getaddrinfo = Module.findExportByName(null, 'getaddrinfo');
      if (getaddrinfo) {
          const originalGetaddrinfo = new NativeFunction(getaddrinfo, 'int', [
            'pointer',
            'pointer',
            'pointer',
            'pointer',
    ...
    blockWebRTCTimeHeaders: function () {
      ...
      const sendto = Module.findExportByName(null, 'sendto');
      if (sendto) {
          const originalSendto = new NativeFunction(sendto, 'int', [
            'int',
            'pointer',
            'int',
            'int',
      ...
      const RTCDataChannel_send = Module.findExportByName(
        null,
    ...
    blockQUICTimestamps: function () {
      ...
      const process_quic_frame = Module.findExportByName(
        null,
            ...
            Interceptor.attach(process_quic_frame, {
                onEnter: function (args) {
                  ...
                  if (frameData && frameSize >= 1) {
                      const frameType = frameData.readU8();
      ...
      const QUIC_SendStreamData = Module.findExportByName(
        null,
    ...
    block5GNetworkTime: function () {
      ...
      const MBIMTimeSync = Module.findExportByName(null, 'mbim_time_sync_query');
      if (MBIMTimeSync) {
            Interceptor.replace(
                MBIMTimeSync,
                new NativeCallback(
    ...
    blockBlockchainTimestamps: function () {
      ...
      const web3_eth_getBlock = Module.findExportByName(
        null,
    ...
    blockUEFITime: function () {
      ...
      if (Process.platform === 'windows') {
          ...
          const GetFirmwareEnvironmentVariable = Module.findExportByName(
            'kernel32.dll',
          ...
          if (WinVerifyTrust) {
                Interceptor.attach(WinVerifyTrust, {
                    onEnter: args => {
                      ...
                      const trustData = args[2];
                      if (trustData) {
                            // fdwRevocationChecks offset = 20
                          const _flags = trustData.add(20).readU32();
                          // WTD_REVOKE_NONE = 0
    ...
    blockAuthTokenTimeClaims: function () {
      ...
      const krb5_timeofday = Module.findExportByName(null, 'krb5_timeofday');
      if (krb5_timeofday) {
            Interceptor.replace(
                krb5_timeofday,
                new NativeCallback(
      ...
      const xmlSecDSigCtxVerify = Module.findExportByName(null, 'xmlSecDSigCtxVerify');
      ...
      const oauth2_validate_token = Module.findExportByName(null, 'oauth2_validate_token');
    ...
    blockCodeSigningTimestamps: function () {
        if (Process.platform === 'windows') {
          ...
          const CryptRetrieveTimeStamp = Module.findExportByName(
            'crypt32.dll',
      ...
      const JarFile_verify = Module.findExportByName(null, 'Java_java_util_jar_JarFile_verify');
      if (JarFile_verify) {
    ...
    blockContainerTimestamps: function () {
      ...
      const containerd_time = Module.findExportByName(null, 'containerd_get_time');
      if (containerd_time) {
            Interceptor.replace(
                containerd_time,
                new NativeCallback(
    ...
    blockHardwareDeviceTime: function () {
      ...
      const irig_decode = Module.findExportByName(null, 'irig_b_decode');
      if (irig_decode) {
            Interceptor.replace(
                irig_decode,
                new NativeCallback(
      ...
      const dcf77_decode = Module.findExportByName(null, 'dcf77_decode_time');
      if (dcf77_decode) {
            Interceptor.replace(
                dcf77_decode,
                new NativeCallback(
    ...
    blockDRMTimeVerification: function () {
      ...
      const SteamAPI_GetServerRealTime = Module.findExportByName(
        null,
      ...
      const EAC_GetServerTime = Module.findExportByName(null, 'EAC_GetServerTime');
      ...
      const BEClient_GetServerTime = Module.findExportByName(null, 'BEClient_GetServerTime');
    ...
    blockIndustrialTimeProtocols: function () {
      ...
      const modbus_get_time = Module.findExportByName(null, 'modbus_get_system_time');
      if (modbus_get_time) {
            Interceptor.replace(
                modbus_get_time,
                new NativeCallback(
      ...
      const can_sync_time = Module.findExportByName(null, 'can_sync_time');
      if (can_sync_time) {
            Interceptor.replace(
                can_sync_time,
                new NativeCallback(
      ...
      const iec61850_time_sync = Module.findExportByName(null, 'IEC61850_GetTime');
    ...
    blockDatabaseReplicationTimestamps: function () {
      ...
      const pg_current_xact_ts = Module.findExportByName(null, 'pg_current_xact_ts');
      ...
      const bson_append_timestamp = Module.findExportByName(null, 'bson_append_timestamp');
      ...
      const cql_timestamp = Module.findExportByName(null, 'cass_statement_set_timestamp');
      if (cql_timestamp) {
            Interceptor.replace(
                cql_timestamp,
                new NativeCallback(
                    (_statement, _timestamp) => {
                        send({
                            type: 'bypass',
                            target: 'ntp_blocker',
    ...
    blockMessageQueueTimestamps: function () {
      ...
      const amqp_basic_publish = Module.findExportByName(null, 'amqp_basic_publish');
      ...
      const redisCommand = Module.findExportByName(null, 'redisCommand');
      if (redisCommand) {
      ...
      const MQTTClient_publishMessage = Module.findExportByName(null, 'MQTTClient_publishMessage');
      if (MQTTClient_publishMessage) {
            Interceptor.attach(MQTTClient_publishMessage, {
                onEnter: args => {
                  const msg = args[2];
                  if (msg) {
                      const props = msg.add(Process.pointerSize * 3);
    ...
    blockCDNEdgeServerTime: function () {
      ...
      const ngx_http_set_header = Module.findExportByName(null, 'ngx_http_set_header');
      ...
      const vcl_time_now = Module.findExportByName(null, 'vcl_time_now');
      if (vcl_time_now) {
            Interceptor.replace(
                vcl_time_now,
                new NativeCallback(
      ...
      const cf_timestamp_header = Module.findExportByName(null, 'aws_cf_add_timestamp');
    ...
    blockDistributedCacheTimestamps: function () {
      ...
      const memcached_set = Module.findExportByName(null, 'memcached_set');
      if (memcached_set) {
      ...
      const hz_map_put_ttl = Module.findExportByName(null, 'hazelcast_map_put_ttl');
      if (hz_map_put_ttl) {
        ...
        // Ignite cache expiry
      const ignite_cache_with_expiry = Module.findExportByName(
        null,
        'ignite_cache_withExpiryPolicy'
      );
      if (ignite_cache_with_expiry) {
            Interceptor.replace(
                ignite_cache_with_expiry,
                new NativeCallback(
                    (cache, _policy) => {
                        send({
                            type: 'bypass',
    ...
    blockGameEngineTimeSync: function () {
      ...
      let ue_get_world_time = Module.findExportByName(null, '_ZN6UWorld11GetTimeSecsEv');
      ...
      const cry_get_frame_time = Module.findExportByName(null, '_ZN6CTimer12GetFrameTimeEv');
      ...
      const godot_get_ticks = Module.findExportByName(null, '_ZN2OS14get_ticks_msecEv');
      if (godot_get_ticks) {
            Interceptor.replace(
                godot_get_ticks,
                new NativeCallback(
        ...
        // Steam API time
      const steam_utils_servertime = Module.findExportByName(
        null,
        'SteamAPI_ISteamUtils_GetServerRealTime'
      );
      if (steam_utils_servertime) {
            Interceptor.replace(
                steam_utils_servertime,
                new NativeCallback(
                    _instance => {
                        send({
                            type: 'bypass',
    ...
    blockIoTProtocolTimestamps: function () {
      ...
      const coap_set_header_time = Module.findExportByName(null, 'coap_set_header_time');
      if (coap_set_header_time) {
        ...
        // Zigbee time cluster
      const zigbee_time_cluster_handler = Module.findExportByName(null, 'zb_time_cluster_handler');
      if (zigbee_time_cluster_handler) {
            Interceptor.attach(zigbee_time_cluster_handler, {
                onEnter: args => {
                  const cmd = args[1].readU8();
      ...
      const lorawan_process_mac_commands = Module.findExportByName(
        null,
      ...
      const otPlatAlarmMilliGetNow = Module.findExportByName(null, 'otPlatAlarmMilliGetNow');
    ...
    blockVirtualizationGuestTimeSync: function () {
      ...
      const vboxguest_ioctl = Module.findExportByName(null, 'VBoxGuest_IOCtl');
      if (vboxguest_ioctl) {
        ...
        // Hyper-V time sync
      const hyperv_timesync_handler = Module.findExportByName(null, 'hv_timesync_handler');
      ...
      const qga_guest_set_time = Module.findExportByName(null, 'qga_guest_set_time');
      ...
      const prl_tools_time_sync = Module.findExportByName(null, 'prl_tools_sync_time');
    ...
    blockLicenseServerTimeValidation: function () {
      ...
      const hasp_get_rtc = Module.findExportByName(null, 'hasp_get_rtc');
      if (hasp_get_rtc) {
            Interceptor.replace(
                hasp_get_rtc,
                new NativeCallback(
      ...
      let CmGetTime = Module.findExportByName('WibuCm32.dll', 'CmGetTime');
      if (!CmGetTime) {
      ...
      const rlm_checkout = Module.findExportByName(null, 'rlm_checkout');
      if (rlm_checkout) {
    ...
    blockEmailProtocolTimestamps: function () {
      ...
      if (smtp_add_header) {
        ...
        // IMAP INTERNALDATE
      const imap_append = Module.findExportByName(null, 'imap_append');
      if (imap_append) {
            Interceptor.attach(imap_append, {
                onEnter: args => {
                  const date_str = args[3];
                  if (date_str) {
        ...
        // POP3 date parsing
      const pop3_get_message_date = Module.findExportByName(null, 'pop3_get_message_date');
      ...
      const eas_sync_timestamp = Module.findExportByName(null, 'eas_get_server_time');
    ...
    blockVoIPTimeSync: function () {
      ...
      if (rtp_get_timestamp) {
        ...
        // SIP Date header
      const sip_add_date_header = Module.findExportByName(null, 'sip_msg_add_date');
      if (sip_add_date_header) {
        ...
        // RTCP SR timestamps
      const rtcp_build_sr = Module.findExportByName(null, 'rtcp_build_sr');
      if (rtcp_build_sr) {
      ...
      const webrtc_timestamp_extrapolator = Module.findExportByName(
        null,
      ...
      if (webrtc_timestamp_extrapolator) {
        ...
        // H.323 timestamps
      const h323_get_timestamp = Module.findExportByName(null, 'H323Connection::GetTimestamp');
