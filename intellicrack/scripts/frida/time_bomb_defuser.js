    ...
    this.performanceMonitor = {
      ...
      measure: (hookName, fn) => {
        ...
        if (this.performanceMonitor.hookOverhead[hookName].length > 100) {
          const avg = this.calculateAverage(this.performanceMonitor.hookOverhead[hookName]);
          if (avg > 1000) {
            // 1ms threshold
  ...
  getSpoofedTime: function () {
    ...
    if (this.config.timeProgression.enabled) {
      ...
      if (this.config.timeProgression.randomVariation > 0) {
        const variation = (Math.random() - 0.5) * this.config.timeProgression.randomVariation;
