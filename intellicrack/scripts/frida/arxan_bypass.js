...
const ArxanBypass = {
    ...
    bypassAntiDebug: function() {
        ...
        if (ntQueryInfo) {
            ...
            this.hooks.installed++;
            this.log('info', 'Bypassed NtQueryInformationProcess (DebugPort, DebugObjectHandle, DebugFlags)');
        }
        ...
        if (outputDebugString) {
            Interceptor.replace(outputDebugString, new NativeCallback(function(lpOutputString) {
                return;
            }, 'void', ['pointer']));
            this.hooks.installed++;
        ...
        try {
            const peb = Process.pointerSize === 8
    ...
    bypassIntegrityChecks: function() {
        ...
        const cryptVerifySignature = Module.findExportByName('Advapi32.dll', 'CryptVerifySignature');
        if (cryptVerifySignature) {
            Interceptor.replace(cryptVerifySignature, new NativeCallback(function(hHash, pbSignature, dwSigLen, hPubKey, sDescription, dwFlags) {
                return 1;
        ...
        if (winVerifyTrust) {
            Interceptor.replace(winVerifyTrust, new NativeCallback(function(hwnd, pgActionID, pWVTData) {
                return 0;
        ...
        const certVerifyChain = Module.findExportByName('Crypt32.dll', 'CertVerifyCertificateChainPolicy');
        if (certVerifyChain) {
    ...
    bypassRASP: function() {
        ...
        if (setUnhandledExceptionFilter) {
            Interceptor.replace(setUnhandledExceptionFilter, new NativeCallback(function(lpTopLevelExceptionFilter) {
                return ptr(0);
            }, 'pointer', ['pointer']));
            this.hooks.installed++;
    ...
    patchInlineChecks: function() {
        ...
        modules.forEach((module) => {
            if (module.name.toLowerCase().includes('arxan') ||
