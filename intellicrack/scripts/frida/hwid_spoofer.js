...
class HwidSpooferEnhanced {
    ...
    initializeAdvancedHardwareFingerprinting() {
                        ...
                        try {
                            ...
                            if (vtType === 8) { // VT_BSTR
                                ...
                                if (bstrPtr && !bstrPtr.isNull()) {
                                    ...
                                    if (str && (str.includes('Win32_') || str.includes('ROOT\\CIMV2') || str.includes('SELECT * FROM'))) {
                                        this.wmiQuery = str;
                                        this.spoofWmi = true;
                                    }
                                }
        ...
        const setupapi = Module.findExportByName('setupapi.dll', 'SetupDiGetDeviceRegistryPropertyW');
        if (setupapi) {
        ...
        if (advapi32) {
            Interceptor.attach(advapi32, {
                onEnter: function(args) {
                    ...
                    if (keyName && (keyName.includes('TPM') || keyName.includes('TBS') || keyName.includes('Platform'))) {
                        this.tpmQuery = true;
        ...
        if (ntdll) {
            Interceptor.attach(ntdll, {
                onEnter: function(args) {
                    ...
                    if (infoClass === 76) { // SystemFirmwareTableInformation
                        this.spoofSmbios = true;
                    }
                },
                onLeave: function(retval) {
                    if (this.spoofSmbios && retval.toInt32() === 0 && this.buffer && !this.buffer.isNull()) {
                        // Spoof SMBIOS structure with generic hardware info
                        const smbiosHeader = new Uint8Array([
                            0x53, 0x4D, 0x42, 0x49, // "SMBI"
                            0x4F, 0x53, 0x00, 0x00, // "OS\0\0"
                            0x20, 0x00, 0x00, 0x00, // Length
                            0x01, 0x02, 0x03, 0x04  // Version info
                        ]);
        ...
        if (regQueryEx) {
            Interceptor.attach(regQueryEx, {
                onEnter: function(args) {
                    ...
                    if (valueName && (valueName.includes('Manufacturer') || valueName.includes('Product') || valueName.includes('Version'))) {
    ...
    setupCpuAdvancedSpoofing() {
        ...
        const cpuidHook = Interceptor.attach(Module.findExportByName('kernel32.dll', 'IsProcessorFeaturePresent'), {
            onEnter: function(args) {
                this.feature = args[0].toInt32();
            },
            onLeave: function(retval) {
                // Spoof specific CPU features
                if (this.feature === 6 || this.feature === 10 || this.feature === 17) { // PAE, XMMI64, NX
        ...
        if (ntdll) {
            Interceptor.attach(ntdll, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() === 0 && this.outputBuffer && !this.outputBuffer.isNull()) {
                        // Spoof power and thermal information
                        if (this.infoLevel === 12 || this.infoLevel === 60) { // ProcessorInformation or ThermalInformation
        ...
        const getCacheInfo = Module.findExportByName('kernel32.dll', 'GetLogicalProcessorInformation');
        if (getCacheInfo) {
    ...
    initializeGpuHardwareSpoofer() {
        ...
        if (setupapi) {
            Interceptor.attach(setupapi, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() !== 0 && this.deviceInfoData && !this.deviceInfoData.isNull()) {
    ...
    setupUsbDeviceEnumerationSpoof() {
        ...
        const getDeviceProperty = Module.findExportByName('setupapi.dll', 'SetupDiGetDevicePropertyW');
        ...
        if (kernel32) {
            Interceptor.attach(kernel32, {
                ...
                onLeave: function(retval) {
                    if (this.usbIoctl && retval.toInt32() !== 0 && this.outputBuffer && !this.outputBuffer.isNull()) {
    ...
    initializeDisplayAdapterSpoofing() {
        ...
        if (user32) {
            Interceptor.attach(user32, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() !== 0 && this.displayDevice && !this.displayDevice.isNull()) {
    ...
    initializeMemoryConfigurationSpoof() {
        ...
        if (kernel32) {
            Interceptor.attach(kernel32, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() !== 0 && this.memoryStatus && !this.memoryStatus.isNull()) {
        ...
        const getLogicalProcessorInfo = Module.findExportByName('kernel32.dll', 'GetLogicalProcessorInformationEx');
        if (getLogicalProcessorInfo) {
            Interceptor.attach(getLogicalProcessorInfo, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() !== 0 && this.buffer && !this.buffer.isNull()) {
                        if (this.relationshipType === 3) { // RelationCache
        ...
        const getNumaNodeProcessorMask = Module.findExportByName('kernel32.dll', 'GetNumaNodeProcessorMaskEx');
        if (getNumaNodeProcessorMask) {
            Interceptor.attach(getNumaNodeProcessorMask, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() !== 0 && this.processorMask && !this.processorMask.isNull()) {
            ...
            Interceptor.attach(ntdll, {
                ...
                onLeave: function(retval) {
                    if (retval.toInt32() === 0 && this.infoBuffer && !this.infoBuffer.isNull()) {
                        if (this.infoClass === 8) { // SystemProcessorPerformanceInformation
                        ...
                        } else if (this.infoClass === 2) { // SystemPerformanceInformation
        ...
        if (diskPerf) {
            Interceptor.attach(diskPerf, {
                onEnter: function(args) {
                    ...
                    if (this.ioControlCode === 0x00070020) { // IOCTL_DISK_PERFORMANCE
                        this.diskPerfQuery = true;
                    }
                },
                onLeave: function(retval) {
                    if (this.diskPerfQuery && retval.toInt32() !== 0 && this.outputBuffer && !this.outputBuffer.isNull()) {
