    ...
    initializePatchingEngine: function() {
        this.patchingEngine = {
            ...
            rollbackPatch: function(patchId) {
                ...
                const originalBytes = BinaryPatcher.state.rollbackData.get(patch.address.toString());
                if (!originalBytes) {
                    return false;
                }
        ...
        this.architectures = {
            ...
            x86_64: {
                ...
                generateJmp: function(from, to) {
                    ...
                    return [
                        0x48, 0xB8, ...this.int64ToBytes(absAddr), // MOV RAX, address
                        0xFF, 0xE0  // JMP RAX
                    ];
                },
                ...
                patchReturn: function(address, returnValue) {
                    ...
                    const originalSize = Memory.readU8(address.add(bytes.length - 1)) === 0xC3 ? bytes.length : 10;
            ...
            arm64: {
                ...
                generateBranch: function(from, to) {
                    ...
                    // Use ADRP + BR for long jumps
                    const page = (to.toInt64() >> 12) << 12;
                    const pageOffset = to.toInt64() & 0xFFF;
                    const adrp = 0x90000000 | ((page >> 12) & 0x1FFFFF) << 5;
                    const br = 0xD61F0000;

                    return [...this.int32ToBytes(adrp), ...this.int32ToBytes(br)];
                },
            ...
            wasm: {
                ...
                patchFunction: function(funcIndex, returnValue) {
                    return [
                        0x41, returnValue, // i32.const value
                        0x0F // return
                    ];
                },
        ...
        this.formatHandlers = {
            ...
            pe: {
                ...
                parseHeaders: function(buffer) {
                    ...
                    const sections = this.parseSections(buffer, dos.e_lfanew, nt.FileHeader.NumberOfSections);
            ...
            macho: {
                ...
                findCodeSignature: function(buffer) {
                    ...
                    const view = new DataView(buffer);
                    let offset = header.magic === 0xFEEDFACF ? 32 : 28;
                    ...
                    for (let i = 0; i < substituted.length - 1; i++) {
                        ...
                        if (substituted[i] === 0x89 || substituted[i] === 0x8B) {
                            ...
                            if (regMap[reg] !== undefined) {
                                substituted[i + 1] = (modRM & 0xC7) | (regMap[reg] << 3);
                            }
                        }
                    }
                        ...
                        if (func) {
                            Interceptor.attach(func, {
                                onEnter: function(args) {
                                    ...
                                    // Check if reading our patch
                                    if (targetAddr.compare(address) >= 0 &&
                                        targetAddr.compare(address.add(size)) < 0) {
                                        // Redirect to original bytes
                                        const original = BinaryPatcher.state.rollbackData.get(address.toString());
                                        if (original) {
                                            args[1] = Memory.alloc(original.length);
                                            Memory.writeByteArray(args[1], original);
                                        }
                                    }
                                }
                            });
    ...
    applyPatch: function(target) {
        ...
        try {
            ...
            if (this.config.antiDetection.polymorphic) {
                const variants = this.antiDetection.polymorphic.generateVariants({data: patchData});
