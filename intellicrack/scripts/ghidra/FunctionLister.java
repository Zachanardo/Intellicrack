...
public class FunctionLister extends GhidraScript {
  ...
  private void analyzeWithDecompiler(Function func, FunctionMetrics metrics) {
    try {
      ...
      // Decompilation failed, use basic analysis only
    }
  }
  ...
  private void checkForPatterns(Function func, FunctionMetrics metrics) {
    ...
    while (instIter.hasNext()) {
      ...
      if (mnemonic.equals("xor") || mnemonic.equals("not") || mnemonic.equals("neg")) {
        hasObfuscation = true;
      }
    }
    ...
    for (FunctionMetrics metrics : functionMetrics.values()) {
      ...
      for (String called : metrics.calledFunctions) {
        for (String dangerous : dangerousFuncs) {
          if (called.toLowerCase().contains(dangerous)) {
      ...
      if (metrics.name.toLowerCase().contains("alloc")
          || metrics.name.toLowerCase().contains("size")) {
  ...
  private void generateSummaryReport() {
    ...
    for (FunctionMetrics metrics : functionMetrics.values()) {
      if (metrics.hasLoops) withLoops++;
      if (metrics.hasRecursion) withRecursion++;
      if (metrics.usesIndirectCalls) withIndirectCalls++;
      if (metrics.hasVulnerabilities) withVulnerabilities++;
    }
  ...
  private void analyzeCodeUnits() {
    ...
    while (codeUnitIter.hasNext() && !monitor.isCancelled()) {
      ...
      if (comment != null && comment.toLowerCase().contains("license")) {
  ...
  private void analyzeInstructionPattern(Instruction inst) {
    ...
    if (mnemonic.equals("int3") || mnemonic.equals("int") || mnemonic.equals("icebp")) {
    ...
    // Check for obfuscation patterns
    if (mnemonic.equals("jmp") && inst.getNext() != null) {
  ...
  private boolean analyzeX86SpecificPatterns(
    ...
    while (instIter.hasNext()) {
      ...
      if (mnemonic.startsWith("rep")
          ...
          || mnemonic.equals("lodsb")
          || mnemonic.equals("cmpsb")
          || mnemonic.equals("pusha")
          || mnemonic.equals("popa")) {
        hasX86Specific = true;
        metrics.vulnerabilities.add("Uses x86-specific string/stack instructions");
      }
      ...
      if (mnemonic.startsWith("f")
          && (mnemonic.contains("st") || mnemonic.equals("fld") || mnemonic.equals("fst"))) {
        hasX86Specific = true;
      }
    }
  ...
  private boolean analyzeMIPSSpecificPatterns(
    ...
    while (instIter.hasNext()) {
      ...
      if (mnemonic.equals("jalr")
          ...
          || mnemonic.equals("syscall")
          || mnemonic.equals("break")) {
        hasMIPSSpecific = true;
        metrics.vulnerabilities.add("Uses MIPS-specific instructions");
      }

      // Check for delay slot usage
      if (mnemonic.equals("nop") && inst.getNext() != null) {
        String nextMnemonic = inst.getNext().getMnemonicString().toLowerCase();
    ...
    for (FunctionMetrics metrics : functionMetrics.values()) {
      ...
      while (instIter.hasNext()) {
        ...
        if (mnemonic.startsWith("sse")
          ...
          cryptoInstructions++;
        } else if (mnemonic.equals("rdrand")
            || mnemonic.equals("rdseed")
            || mnemonic.equals("rdtsc")
            || mnemonic.equals("rdtscp")) {
  ...
  private boolean checkX86Vulnerabilities(Function func, FunctionMetrics metrics) {
    ...
    while (instIter.hasNext()) {
      ...
      if (mnemonic.equals("gets") || mnemonic.equals("strcpy")) {
        metrics.vulnerabilities.add("Uses dangerous string function: " + mnemonic);
        hasVuln = true;
      }

      // Check for stack manipulation without bounds checking
      if (mnemonic.equals("rep") && inst.getNext() != null) {
        String next = inst.getNext().getMnemonicString().toLowerCase();
        if (next.equals("stosb") || next.equals("movsb")) {
  ...
  private boolean checkBufferOverflowPatterns(
    ...
    while (instIter.hasNext()) {
      ...
      if (inst.getMnemonicString().toLowerCase().contains("mov")) {
  ...
  private void analyzeEnumType(ghidra.program.model.data.Enum enumType) {
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      if (name.contains("admin")
        ...
        println("[DataType Finding] Security-related enum: " + enumType.getName() + "." + names[i]);
      }
    }
  }
    ...
    try {
      ...
      if (bytesRead > 0) {
        ...
        if (containsShellcodePatterns(data)) {
          createBookmark(start, "Security", "Potential shellcode pattern in " + block.getName());
        }

        // Check for string patterns
        if (containsSuspiciousStrings(data)) {
          createBookmark(start, "Security", "Suspicious strings in " + block.getName());
        }
      }
  ...
  private boolean isDangerousStringFunction(String funcName) {
    ...
    for (String dangerous_func : dangerous) {
      if (funcName.toLowerCase().contains(dangerous_func)) {
        return true;
      }
    }
  ...
  private void analyzeStringFunctionUsage(
    ...
    while (instIter.hasNext()) {
      ...
      if (inst.getMnemonicString().toLowerCase().equals("call")) {
        String target = inst.getDefaultOperandRepresentation(0);
        if (target != null && target.toLowerCase().contains(calledFunc.toLowerCase())) {
    ...
    for (Map.Entry<Address, List<Reference>> refEntry : referenceCache.entrySet()) {
      ...
      if (codeUnit != null) {
        ...
        for (Reference ref : refs) {
          ...
          if (type.isCall()) result.callReferences++;
          if (type.isData()) result.dataReferences++;
          if (type.isIndirect()) result.indirectReferences++;
        }
  ...
  private class CombinedAnalysisResult {
    Address address;
    int referenceCount;
    CodeUnit codeUnit;
  ...
  private boolean isSuspiciousCombination(CombinedAnalysisResult result) {
    ...
    if (result.referenceCount >= 5 && result.isControlFlow) {
      return true;
    }

    // Moderate references with indirect references (potential obfuscation)
    if (result.referenceCount >= 3 && result.indirectReferences > 0) {
      return true;
    }
  ...
  private boolean isLicenseValidationCandidate(CombinedAnalysisResult result, Function func) {
    ...
    if (result.referenceCount >= 5 && result.mnemonic.toLowerCase().contains("cmp")) {
      return true;
    }
