...
public class NetworkAnalysis extends GhidraScript {
    ...
    private String extractPotentialIPAroundDot(Address dotAddr) {
        try {
            ...
            memory.getBytes(readAddr, chunk);
    ...
    private void analyzeNetworkFunction(Function func) {
        ...
        for (Parameter param : params) {
            ...
            if (paramType.getName().contains("SOCKET") || 
                paramType.getName().contains("HANDLE")) {
        ...
        if (returnType.getName().contains("SOCKET") ||
            returnType.getName().contains("BOOL")) {
            ...
            for (Reference ref : refs) {
                if (ref.getReferenceType().isData()) {
                    ...
                    if (data != null && data.hasStringValue()) {
                        ...
                        if (value.contains("http") || value.contains("tcp") ||
                            value.contains("udp") || value.contains("socket")) {
    ...
    private void mapNetworkBuffers() {
        ...
        for (Address bufferAddr : bufferLocations) {
            try {
                ...
                MemoryBlock block = memory.getBlock(bufferAddr);
                if (block != null) {
                    // Create address range for buffer region
                    Address blockStart = block.getStart();
                    Address blockEnd = block.getEnd();
                    AddressRange range = new ghidra.program.database.map.AddressRangeImpl(
                        blockStart, blockEnd);
    ...
    private void parseNetworkStructures() {
        ...
        while (allTypes.hasNext() && !monitor.isCancelled()) {
            ...
            if (typeName.contains("sock") || typeName.contains("addr") ||
                typeName.contains("http") || typeName.contains("tcp")) {
                
                if (dt instanceof Structure) {
                    Structure struct = (Structure) dt;
                    println("    Found network structure: " + dt.getName() + 
                    ...
                    for (int i = 0; i < struct.getNumComponents(); i++) {
                        ...
                        if (comp.getFieldName() != null) {
                            if (comp.getFieldName().contains("port") ||
                                comp.getFieldName().contains("addr")) {
                                println("      - " + comp.getFieldName() + ": " +
                    ...
                    println("    Found network enum: " + dt.getName() + 
        ...
        while (dataIter.hasNext() && !monitor.isCancelled()) {
            ...
            if (typeName.contains("sock") || typeName.contains("addr")) {
                ...
                if (func != null && networkFunctions.contains(func.getEntryPoint())) {
                    println("    Network structure " + dataType.getName() + 
