...
from .syscalls import inject_using_syscalls, get_direct_syscalls
            ...
            for reloc in pe.DIRECTORY_ENTRY_BASERELOC:
                ...
                for entry in reloc.entries:
                    ...
                    success = KERNEL32.ReadProcessMemory(
                        ...
                        8 if pe.PE_TYPE == pefile.OPTIONAL_HEADER_MAGIC_PE_PLUS else 4,
        ...
        try:
            ...
            for entry in pe.DIRECTORY_ENTRY_IMPORT:
                ...
                for imp in entry.imports:
                    ...
                    addr_bytes = struct.pack('<Q' if pe.PE_TYPE == pefile.OPTIONAL_HEADER_MAGIC_PE_PLUS else '<I', 
                                           func_addr)
                    bytes_written = ctypes.c_size_t(0)
    ...
    def _execute_tls_callbacks(self, process_handle: int, pe: Any, 
        ...
        try:
            ...
            while True:
                ...
                success = KERNEL32.ReadProcessMemory(
                    process_handle,
                    remote_base + callback_addr - pe.OPTIONAL_HEADER.ImageBase,
                ...
                self._create_remote_thread(
                    process_handle,
                    remote_base + addr_value.value - pe.OPTIONAL_HEADER.ImageBase,
                    remote_base
                )
                
                callback_addr += 8 if pe.PE_TYPE == pefile.OPTIONAL_HEADER_MAGIC_PE_PLUS else 4
    ...
    def is_dll_64bit(self, dll_path: str) -> Optional[bool]:
        ...
        try:
            ...
            if pe.FILE_HEADER.Machine == 0x8664:  # IMAGE_FILE_MACHINE_AMD64
                ...
                logger.warning(f"Unknown machine type: {hex(pe.FILE_HEADER.Machine)}")
                return None
