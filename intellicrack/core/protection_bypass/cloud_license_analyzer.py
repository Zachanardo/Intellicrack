...
class CloudLicenseAnalyzer:
        ...
        if (Process.platform === 'windows') {
            ...
            Interceptor.attach(WinHttpOpen, {
                onEnter: function(args) {
                    ...
                    args[2] = Memory.allocUtf16String('http://127.0.0.1:""" + str(self.proxy_port) + """');
                },
                ...
                Interceptor.attach(curl_easy_setopt, {
                    onEnter: function(args) {
                        ...
                        if (option === CURLOPT_PROXY) {
                            ...
                            args[2] = Memory.allocUtf8String('http://127.0.0.1:""" + str(self.proxy_port) + """');
                        }
                    }
                });
            }
    ...
    def analyze_endpoint(self, request: mitmproxy.http.Request,
                         response: mitmproxy.http.Response) -> CloudEndpoint:
    ...
    def extract_license_tokens(self, request: mitmproxy.http.Request,
                              response: mitmproxy.http.Response) -> List[LicenseToken]:
    ...
    def _generate_jwt_token(self, issuer: str = 'intellicrack', subject: str = 'user',
                           audience: str = None, expires_in: int = 3600,
                           claims: Dict[str, Any] = None, **kwargs) -> str:
        now = datetime.utcnow()
    ...
    def export_analysis(self, filepath: Path) -> bool:
        ...
        try:
            analysis_data = {
                ...
                'endpoints': {k: self._serialize_endpoint(v)
                             for k, v in self.discovered_endpoints.items()},
                'tokens': {k: self._serialize_token(v)
                          for k, v in self.license_tokens.items()},
...
class CloudInterceptor:
    ...
    def response(self, flow: mitmproxy.http.HTTPFlow):
        ...
        endpoint = self.analyzer.analyze_endpoint(request, response)

        tokens = self.analyzer.extract_license_tokens(request, response)
        for token in tokens:
            token_key = f"{token.token_type}:{hashlib.md5(token.value.encode()).hexdigest()[:8]}"
            self.analyzer.license_tokens[token_key] = token

        if self._should_modify_response(request, response):
            self._modify_response(flow)

    def _should_modify_response(self, request: mitmproxy.http.Request,
                               response: mitmproxy.http.Response) -> bool:
...
class CloudLicenseBypasser:
    ...
    def bypass_license_check(self, target_url: str) -> bool:
        ...
        for endpoint_key, endpoint in self.analyzer.discovered_endpoints.items():
    ...
    def _get_valid_token(self, endpoint: CloudEndpoint) -> Optional[LicenseToken]:
        for token_key, token in self.analyzer.license_tokens.items():
