...
class BinaryFeatureExtractor:
    ...
    def _extract_section_features(self, data: bytes) -> dict[str, float]:
        ...
        try:
            ...
            for section in sections:
                ...
                if section_name and not any(
                    common in section_name
                    for common in ['.text', '.data', '.rdata', '.bss', '.idata', '.edata', '.rsrc']
                ):
                    unusual_names += 1
    ...
    def _extract_import_features(self, data: bytes) -> dict[str, float]:
        ...
        try:
            ...
            if len(imports) > 0 and len(unique_dlls) > 0:
    ...
    def _extract_signature_features(self, data: bytes) -> dict[str, float]:
        ...
        try:
            ...
            for protector, patterns in self.PROTECTOR_PATTERNS.items():
                ...
                if 'entry_point_sig' in patterns:
                    if entry_point.startswith(patterns['entry_point_sig']):
                        score += 0.5
                        matches.append('entry_point_match')

                if 'timestamp' in patterns:
                    if timestamp == patterns['timestamp']:
                        score += 0.6
