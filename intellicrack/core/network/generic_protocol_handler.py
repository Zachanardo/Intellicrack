...
class GenericProtocolHandler(LicenseProtocolHandler):
    ...
    def handle_connection(self, socket: Any, initial_data: bytes) -> None:
        ...
        """
        # Log the request
        self.log_request(initial_data, str(socket.getpeername() if hasattr(socket, 'getpeername') else 'unknown'))
        ...
        self.captured_requests.append({
            ...
            'source': str(socket.getpeername() if hasattr(socket, 'getpeername') else 'unknown')
        })
        ...
        if response:
            try:
                ...
                elif hasattr(socket, 'sendto'):
                    # For UDP-like sockets
                    socket.sendto(response, socket.getpeername())
                    
                self.log_response(response, str(socket.getpeername() if hasattr(socket, 'getpeername') else 'unknown'))
                ...
                self.captured_responses.append({
                    ...
                    'data': response,
                    'hex': response.hex(),
                    'destination': str(socket.getpeername() if hasattr(socket, 'getpeername') else 'unknown')
                })
