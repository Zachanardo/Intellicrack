...
class NetworkTrafficAnalyzer(BaseNetworkAnalyzer):
    ...
    def start_capture(self, interface: str | None = None) -> bool:
        ...
        try:
            ...
            self.logger.info("Started packet capture on %s", interface or "default interface")
            return True
                ...
                if os.name == "nt":  # Windows
                    ...
                    if interface:
                        ...
                        try:
                            ...
                            self.logger.warning("Could not bind to interface %s, using default: %s", interface, e)
                ...
                try:
                    while self.capturing and not (packet_count and packets_captured >= packet_count):
                        ...
                        if timeout and (current_time - start_time) > timeout:
                            self.logger.info("Capture timeout reached after %.2f seconds", current_time - start_time)
                            break
                    ...
                    self.logger.info(
                        "Socket-based packet capture completed: %s packets in %.2f seconds",
                        packets_captured,
                        capture_stats["capture_time"],
                    )
    ...
    def _process_captured_packet(self, packet_data: bytes) -> None:
        ...
        try:
            ...
            if len(packet_data) >= 20:  # Minimum IP header size
                ...
                if protocol == 6 and len(packet_data) >= ihl + 4:
                    try:
                        ...
                        if src_port in self.license_ports or dst_port in self.license_ports:
                            self.logger.info(
                                "Potential license traffic detected: port %s->%s",
                                src_port,
                                dst_port,
                            )
                    except Exception as e:
    ...
    def _capture_with_pyshark(
        ...
        try:
            ...
            # Log capture start
            self.logger.info("Starting packet capture on %s", interface or "all interfaces")
            ...
            def signal_handler(sig: int, frame: object) -> None:
                ...
                if frame:
                    self.logger.debug(
                        "Signal received at frame: %s",
                        frame.f_code.co_filename if hasattr(frame, "f_code") else "unknown",
                    )
            ...
            for packet in capture.sniff_continuously():
                ...
                try:
                    ...
                    if packets_captured % 100 == 0:
                        ...
                        self.logger.info(
                            "Captured %s packets (%.2f packets/sec), analyzed %s license-related packets",
                            packets_captured,
                            rate,
                            len(self.packets),
                        )
            ...
            self.logger.info(
                "Capture complete. Duration: %.2fs, Total packets: %s, Analyzed packets: %s, "
                "License-related packets: %s, Errors: %s, Rate: %.2f packets/sec",
                duration,
                packets_captured,
                packets_analyzed,
                len(self.packets),
            ...
            if hasattr(packet, "tcp") and hasattr(packet, "ip") and hasattr(packet, "sniff_timestamp") and hasattr(packet, "length"):
                ...
                if hasattr(packet, "tcp") and hasattr(packet.tcp, "payload"):
                    try:
                        ...
                        self.logger.debug("Failed to parse packet payload: %s", e)
                        payload = None

                    # Look for license-related strings in payload
                    if payload and len(payload) > 10 and self._check_payload_for_license_content(payload, conn_key):
    ...
    def _capture_with_scapy(self, interface: str | None = None) -> None:
    ...
    def _generate_visualizations(self, results: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Save figure
            plt.savefig(
                f"{self.config['visualization_dir']}/license_connections_{timestamp}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _assess_threat_level(self, indicators: list[str]) -> str:
        ...
        if len(indicators) >= 3:
            return "high"
        return "medium" if len(indicators) >= 2 else "low"
    ...
    def _calculate_packet_rate(self) -> float:
        ...
        return len(self.packets) / duration if duration > 0 else 0.0
    ...
    def generate_report(self, filename: str | None = None) -> bool:
        ...
        try:
            ...
            html = f"""{get_traffic_html_template()}
