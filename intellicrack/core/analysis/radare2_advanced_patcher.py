    ...
    def redirect_call_target(self, call_address: int, new_target: int) -> PatchInfo:
        ...
        if self.architecture in [Architecture.X86, Architecture.X86_64]:
            ...
            if -0x2000000 <= offset <= 0x1FFFFFF:
                ...
                trampoline = []
                # movz x16, #(target & 0xFFFF)
                trampoline.append(0xD2800010 | ((new_target & 0xFFFF) << 5))
                # movk x16, #((target >> 16) & 0xFFFF), lsl #16
...
def main():
    """Example usage of Radare2AdvancedPatcher"""
    import argparse
    ...
    parser.add_argument("-r", "--return", metavar="FUNC:VALUE", dest="return_value", help="Modify function return value")
