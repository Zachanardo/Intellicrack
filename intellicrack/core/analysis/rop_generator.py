...
try:
    ...
    QInputDialog_TYPE: type[QInputDialog] | None = QInputDialog
except ImportError as e:
    logger.exception("Import error in rop_generator: %s", e)
    PYQT6_AVAILABLE = False
    QInputDialog_TYPE = None
...
class ROPChainGenerator:
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        Args:
            config: Optional configuration dictionary with keys like max_chain_length,
                max_gadget_size, and arch (architecture).
        """
    ...
    def find_gadgets(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Found %d gadgets", len(self.gadgets))
            return True

        except (OSError, ValueError, RuntimeError) as e:
            self.logger.exception("Error finding gadgets: %s", e)
            return False
    ...
    def generate_chains(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Generated %d ROP chains", len(self.chains))
            return True

        except (OSError, ValueError, RuntimeError) as e:
            self.logger.exception("Error generating chains: %s", e)
            return False
    ...
    def _find_real_rop_gadgets(self) -> None:
        ...
        ending in 'ret' or equivalent control transfer instructions. Populates
        the self.gadgets list with discovered ROP gadget objects.
        """
        try:
            # Load and analyze the binary
            binary_data = self._load_binary_data()
            if not binary_data:
                self.logger.exception("Could not load binary data for gadget analysis")
                return
            ...
            self.logger.exception("Error in real gadget finding: %s", e)
    ...
    def _load_binary_data(self) -> bytes | None:
        ...
        try:
            ...
            self.logger.exception("Error loading binary: %s", e)
            return None
    ...
    def _disassemble_binary(self, binary_data: bytes) -> list[dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                self.logger.info("Base address: 0x%x", base_address)
            ...
            try:
                ...
                if binary_data[:2] == b"MZ":  # PE
                    ...
                    if hasattr(lief, "parse"):
                        ...
                        if binary is not None:
                            for section in binary.sections:
                                if hasattr(section, "flags") and hasattr(section, "virtual_address"):
                                    flags_value = section.flags
                                    if isinstance(flags_value, int) and flags_value & 0x4:
                                        sections.append((bytes(section.content), section.virtual_address))
            except ImportError as e:
                self.logger.exception("Import error in rop_generator: %s", e)
    ...
    def _fallback_gadget_search(self) -> None:
        ...
        fallback_gadgets = [
            {
                "address": hex(base_addr + 0x1000),
                "instruction": "pop eax ; ret",
                "type": "pop_reg",
                "size": 2,
            },
            {
                "address": hex(base_addr + 0x1010),
                "instruction": "pop ebx ; ret",
                "type": "pop_reg",
                "size": 2,
            },
            {
                "address": hex(base_addr + 0x1020),
                "instruction": "pop ecx ; ret",
                "type": "pop_reg",
                "size": 2,
            },
            {
                "address": hex(base_addr + 0x1030),
                "instruction": "mov eax, ebx ; ret",
                "type": "mov_reg_reg",
                "size": 3,
            },
            {
                "address": hex(base_addr + 0x1040),
    ...
    def _generate_real_rop_chains(self) -> None:
        ...
        Populates self.chains with generated ROP chain objects.
        """
        try:
            ...
            for target in self.target_functions:
                if chain := self._build_rop_chain_for_target(target):
                    self.chains.append(chain)
                    self.logger.info("Built ROP chain for %s: %d gadgets", target["name"], len(chain["gadgets"]))
                else:
                    self.logger.warning("Failed to build ROP chain for %s", target["name"])

        except Exception as e:
            self.logger.exception("Error in real ROP chain generation: %s", e)
    ...
    def _build_rop_chain_for_target(self, target: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if requirements.get("needs_pivot", False):
                if pivot_gadgets := self._find_pivot_gadgets():
            ...
            self.logger.exception("Error building ROP chain for %s: %s", target.get("name", "unknown"), e)
            return None
    ...
    def _build_real_rop_chain(self, target_info: dict[str, Any], chain_type: str, requirements: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: Dictionary containing the built ROP chain data including gadgets,
                payload, success probability, and constraints met.
        """
        try:
            self.logger.info(
                "Building real ROP chain for target: %s, type: %s",
            ...
            if chain_type == "comparison_bypass":
            ...
            elif chain_type == "shell_execution":
            ...
            if self._validate_chain(chain_data):
                ...
                gadgets_used = chain_data.get("gadgets_used", [])
                gadgets_count = len(gadgets_used) if isinstance(gadgets_used, list) else 0
                self.logger.info(
                    "Successfully built ROP chain with %d gadgets, success probability: %.2f",
                    gadgets_count,
                    chain_data["success_probability"],
                )
                return chain_data
            ...
            self.logger.exception("Failed to build real ROP chain: %s", e)
            return {
                "target": target_info,
                "chain_type": chain_type,
                "gadgets_used": [],
    ...
    def _build_shell_execution_chain(
        self,
        chain_data: dict[str, Any],
        control_gadgets: list[dict[str, Any]],
        stack_gadgets: list[dict[str, Any]],
        target_info: dict[str, Any],
    ) -> dict[str, Any]:
        """Build ROP chain for shell execution.

        Args:
            chain_data: Mutable chain data dictionary to populate.
            control_gadgets: List of control flow gadgets available.
            stack_gadgets: List of stack manipulation gadgets available.
    ...
    def _build_memory_permission_chain(
        self,
        chain_data: dict[str, Any],
        control_gadgets: list[dict[str, Any]],
        stack_gadgets: list[dict[str, Any]],
    ...
    def _build_license_bypass_chain(
        self,
        chain_data: dict[str, Any],
        control_gadgets: list[dict[str, Any]],
        arithmetic_gadgets: list[dict[str, Any]],
        target_info: dict[str, Any],
    ) -> dict[str, Any]:
        ...
        Args:
            chain_data: Mutable chain data dictionary to populate.
            control_gadgets: List of control flow gadgets available.
            arithmetic_gadgets: List of arithmetic/logical gadgets available.
            target_info: Target function information dictionary.
        ...
        if xor_gadgets:
            ...
            chain_data["stack_layout"].extend(
                [
                    {
                        "type": "gadget",
                        "address": xor_gadgets[0]["address"],
                        "purpose": "zero_register",
                    },
    ...
    def _build_comparison_bypass_chain(
        self,
        chain_data: dict[str, Any],
        control_gadgets: list[dict[str, Any]],
        arithmetic_gadgets: list[dict[str, Any]],
        ...
        jmp_gadgets = [
            g for g in control_gadgets if any(j in g.get("disasm", "").lower() for j in ["jmp", "je", "jne", "jz", "jnz", "jl", "jg"])
        ]
        call_gadgets = [g for g in control_gadgets if "call" in g.get("disasm", "").lower()]
        ...
        if control_gadgets:
            ...
            chain_data["stack_layout"].extend(
                [
                    {
                        "type": "gadget",
                        "address": control_gadgets[0]["address"],
                        "purpose": "control_flow",
                    },
                    {
                        "type": "address",
    ...
    def _validate_chain(self, chain_data: dict[str, Any]) -> bool:
        ...
        return not any(item["type"] == "address" and item["value"] == 0 for item in chain_data["stack_layout"])
    ...
    def _find_setup_gadgets(self, requirements: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for reg in required_registers:
            if pop_gadget := self._find_gadget_for_register("pop_reg", reg):
        ...
        if not setup_gadgets and required_registers:
            if generic_pops := [g for g in self.gadgets if g.get("type") == "pop_reg"]:
                setup_gadgets.append(
                    {
                        **generic_pops[0],
    ...
    def _find_argument_gadgets(self, requirements: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if required_args := requirements.get("arguments", []):
            self.logger.debug("Looking for gadgets to satisfy %d argument requirements", len(required_args))

        if mov_gadgets := [g for g in self.gadgets if g.get("type") == "mov_reg_reg"]:
            arg_gadgets.append(
                {
                    **mov_gadgets[0],
    ...
    def _find_pivot_gadgets(self) -> list[dict[str, Any]]:
        ...
        for gadget in self.gadgets:
            ...
            if any(stack_op in instruction for stack_op in ["esp", "rsp", "add", "sub"]) and gadget.get("type") in [
                "arith_reg",
                "mov_reg_reg",
            ]:
    ...
    def _find_execution_gadgets(self, target: dict[str, Any], requirements: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        Args:
            target: Target function information dictionary.
            requirements: Chain requirements dictionary.
        ...
        if call_gadgets := [g for g in self.gadgets if g.get("type") in ["call_reg", "jmp_reg"]]:
            exec_gadgets.append(
                {
                    **call_gadgets[0],
        ...
        elif ret_gadgets := [g for g in self.gadgets if g.get("type") == "ret"]:
            exec_gadgets.append(
                {
                    **ret_gadgets[0],
                    "chain_role": "return",
    ...
    def _find_gadget_for_register(self, gadget_type: str, register: str) -> dict[str, Any] | None:
        ...
        Args:
            gadget_type: Type of gadget to search for (e.g., 'pop_reg').
    ...
    def _build_chain_payload(
        self,
        chain_gadgets: list[dict[str, Any]],
        target: dict[str, Any],
        ...
        Args:
            chain_gadgets: List of gadgets selected for the ROP chain.
            target: Target function information dictionary.
    ...
    def _validate_gadget_chain(self, chain_gadgets: list[dict[str, Any]], requirements: dict[str, Any]) -> bool:
        ...
        Args:
            chain_gadgets: List of gadgets in the chain to validate.
    ...
    def _calculate_chain_complexity(self, chain_gadgets: list[dict[str, Any]]) -> int:
        ...
        for gadget in chain_gadgets:
            ...
            if gadget_type == "pop_reg" or gadget_type not in [
                "mov_reg_reg",
                "arith_reg",
                "call_reg",
                "jmp_reg",
    ...
    def _estimate_success_probability(self, chain_gadgets: list[dict[str, Any]], requirements: dict[str, Any]) -> float:
    ...
    def _fallback_chain_generation(self) -> None:
        ...
        Creates minimal but functional ROP chains using available gadgets
        when primary chain generation methods encounter errors.
        """
        ...
        for target in self.target_functions:
            ...
            if pop_gadgets := [g for g in self.gadgets if g.get("type") == "pop_reg"]:
    ...
    def generate_report(self, filename: str | None = None) -> str | None:
        ...
        html = (
            get_base_html_template("ROP Chain Generation Report", custom_css)
            + f"""
        <body>
    ...
    def clear_analysis(self) -> None:
        ...
        Removes all discovered gadgets, generated chains, and target functions
        to prepare for a new analysis session.
        """
        self.gadgets.clear()
        self.chains.clear()
    ...
    def generate_chain(self, target: str, **kwargs: object) -> list[dict[str, Any]]:
        ...
        if not self.gadgets:
            ...
            if not self.find_gadgets():
                self.logger.exception("Failed to find gadgets")
                return []
        ...
        if all(t["name"] != target_info["name"] for t in self.target_functions):
            self.add_target_function(target_info["name"], target_info.get("address"))

        # Get chain parameters from kwargs
        max_length_obj = kwargs.get("max_length", self.max_chain_length)
        max_length = max_length_obj if isinstance(max_length_obj, int) else self.max_chain_length
        chain_type_obj = kwargs.get("chain_type", "auto")
        chain_type = chain_type_obj if isinstance(chain_type_obj, str) else "auto"
        constraints_obj = kwargs.get("constraints", {})
        ...
        if not chain_data:
            self.logger.exception("Failed to build ROP chain for %s", target)
            return []
    ...
    def _parse_target(self, target: str) -> dict[str, Any]:
        ...
        Args:
            target: Target specification string (function name, address, or library.function).

        Returns:
            dict[str, Any]: Parsed target information including name, address, and type.
        """
        target_info = {
            "name": target,
            "address": None,
        ...
        if target.startswith("0x"):
            try:
                ...
                if addr > 0 and addr < 0xFFFFFFFFFFFFFFFF:
                    ...
                    target_info["address_int"] = str(addr)
                else:
                    self.logger.warning("Invalid address value: %s", target)
            except ValueError as e:
                self.logger.exception("Value error in rop_generator: %s", e)
...
def _setup_rop_generator(app: object) -> ROPChainGenerator | None:
    ...
    Returns:
        ROPChainGenerator | None: Configured generator instance or None if setup failed.
    """
    ...
    if hasattr(app, "binary_path"):
        ...
        if isinstance(binary_path, str) and generator.set_binary(binary_path):
            if hasattr(app, "update_output"):
                app.update_output.emit(f"log_message([ROP Chain Generator] Binary set: {binary_path})")
            return generator
    if hasattr(app, "update_output"):
...
def _configure_architecture_and_targets(app: object, generator: ROPChainGenerator) -> bool:
    ...
    Args:
        app: Application object with update_output signal support.
        generator: ROPChainGenerator instance to configure.

    Returns:
        bool: True if configuration completed successfully, False if cancelled.
    """
...
def _execute_rop_generation_workflow(app: object, generator: ROPChainGenerator) -> bool:
    ...
    Args:
        app: Application object with update_output signal.
        generator: ROPChainGenerator instance to run.

    Returns:
        bool: True if workflow completed successfully, False on failure.
    """
    if hasattr(app, "update_output"):
...
def _process_rop_results(app: object, generator: ROPChainGenerator) -> None:
    ...
    Args:
        app: Application object with update_output and analyze_results attributes.
        generator: ROPChainGenerator instance with completed results.
    """
    ...
    if hasattr(app, "analyze_results"):
        ...
        if isinstance(analyze_results, list):
            ...
            analyze_results.append(f"Total gadgets: {results['summary']['total_gadgets']}")
            analyze_results.append(f"Total chains: {results['summary']['total_chains']}")
            analyze_results.append(f"Total targets: {results['summary']['total_targets']}")

            # Display chains
            for i, chain in enumerate(results["chains"]):
                analyze_results.append(f"\nChain {i + 1}: {chain['description']}")
                analyze_results.append(f"Target: {chain['target']['name']}")
                analyze_results.append(f"Length: {chain['length']} gadgets")

                analyze_results.append("Gadgets:")
                for j, gadget in enumerate(chain["gadgets"]):
                    analyze_results.append(f"  {j + 1}. {gadget['address']}: {gadget['instruction']}")

                analyze_results.append("Payload:")
                for addr in chain["payload"]:
                    analyze_results.append(f"  {addr}")
...
def _handle_rop_report_generation(app: object, generator: ROPChainGenerator) -> None:
    ...
    Args:
        app: Application object with update_output and UI capabilities.
        generator: ROPChainGenerator instance with results to report.
    """
    if PYQT6_AVAILABLE:
...
def run_rop_chain_generator(app: object) -> None:
    ...
    Args:
        app: Application object containing binary path and UI signal handlers.

    Executes the complete ROP chain generation workflow including binary loading,
    architecture configuration, gadget finding, chain generation, and report creation.
    """
