...
import numpy as np
from capstone import Cs, CS_ARCH_X86, CS_MODE_32, CS_MODE_64, CS_OPT_DETAIL
...
class CryptographicRoutineDetector:
        ...
        for opcode, instruction in sha_opcodes.items():
            if opcode in data:
                ...
                detection = CryptoDetection(
                    ...
                    details={'hash_type': 'SHA1' if 'SHA1' in instruction else 'SHA256',
                            'hardware': True}
                )
                self.detections.append(detection)
                ...
                if mul_count >= 6 and add_count >= 4 and sub_count >= 2:
                    ...
                    if 0.8 <= ratio <= 2.0:  # Typical for ECC
                        detection = CryptoDetection(
                            ...
                            details={'multiplications': mul_count,
                                   'additions': add_count,
                ...
                if shift_count >= 4 and xor_count >= 4:
                    ...
                    if 0.5 <= ratio <= 2.0:
                        detection = CryptoDetection(
                            ...
                            details={'shift_operations': shift_count,
    ...
    def _check_sbox_pattern(self, data: bytes, reference: bytes) -> bool:
        ...
        matches = sum(1 for i in range(min(len(data), len(reference)))
    ...
    def _find_crypto_references(self, data: bytes, table_offset: int, detection: CryptoDetection) -> None:
        ...
        for i in range(max(0, table_offset - 0x10000),
                      min(len(data), table_offset + 0x10000)):
