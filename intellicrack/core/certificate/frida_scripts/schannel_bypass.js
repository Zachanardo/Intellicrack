...
if (!sspicli) {
    ...
    try {
        const InitializeSecurityContext = Module.findExportByName(sspicli.name, 'InitializeSecurityContextW') ||
        ...
        if (InitializeSecurityContext) {
            Interceptor.attach(InitializeSecurityContext, {
                onEnter: function(args) {
                    ...
                    const modifiedFlags = (originalFlags | ISC_REQ_MANUAL_CRED_VALIDATION) & ~ISC_REQ_USE_SUPPLIED_CREDS;
                    args[3] = ptr(modifiedFlags);

                    log(`InitializeSecurityContext: Target="${targetName}", Flags: 0x${originalFlags.toString(16)} -> 0x${modifiedFlags.toString(16)}`);
    ...
    try {
        const QueryContextAttributes = Module.findExportByName(sspicli.name, 'QueryContextAttributesW') ||
        ...
        if (QueryContextAttributes) {
            Interceptor.attach(QueryContextAttributes, {
                ...
                onLeave: function(retval) {
                    ...
                    if (this.wantsCert && result === SEC_E_OK) {
                        try {
                            ...
                            if (!certContextPtr.isNull()) {
                                ...
                                const pbCertEncoded = certContextPtr.add(Process.pointerSize).readPointer();
                                const cbCertEncoded = certContextPtr.add(Process.pointerSize * 2).readU32();

                                let certData = null;
                                if (!pbCertEncoded.isNull() && cbCertEncoded > 0 && cbCertEncoded < 0x10000) {
                                    try {
                                ...
                                const interceptedCert = {
                                    ...
                                    certData: certData ? Array.from(new Uint8Array(certData)).slice(0, 256) : null,
                                    bypassed: true
                                };
                                ...
                                log(`QueryContextAttributes: Intercepted certificate - Size: ${cbCertEncoded} bytes, Encoding: 0x${dwCertEncodingType.toString(16)}`);
                    ...
                    if (result !== SEC_E_OK) {
                        log(`QueryContextAttributes: Failed with 0x${result.toString(16)}, forcing success`);
    ...
    try {
        const AcceptSecurityContext = Module.findExportByName(sspicli.name, 'AcceptSecurityContext');
        if (AcceptSecurityContext) {
            Interceptor.attach(AcceptSecurityContext, {
                onEnter: function(args) {
                    ...
                    log(`AcceptSecurityContext: Modified flags 0x${originalFlags.toString(16)} -> 0x${modifiedFlags.toString(16)}`);
                },
                onLeave: function(retval) {
                    const result = retval.toInt32();
        ...
        if (EncryptMessage) {
            Interceptor.attach(EncryptMessage, {
                ...
                onLeave: function(retval) {
                    const result = retval.toInt32();
                    if (result !== SEC_E_OK) {
                        log(`EncryptMessage: Failed with 0x${result.toString(16)}, allowing to proceed`);
                    }
                }
            });
        ...
        if (DecryptMessage) {
            Interceptor.attach(DecryptMessage, {
                ...
                onLeave: function(retval) {
                    const result = retval.toInt32();
                    if (result !== SEC_E_OK) {
                        log(`DecryptMessage: Failed with 0x${result.toString(16)}, allowing to proceed`);
                    }
                }
            });
    ...
    try {
        ...
        if (crypt32) {
            const SslCrackCertificate = Module.findExportByName('crypt32.dll', 'SslCrackCertificate');
