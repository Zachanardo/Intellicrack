...
class WebSocketEventStream:
    ...
    async def _send_statistics(self, websocket: WebSocketServerProtocol):
    ...
    async def _send_history(self, websocket: WebSocketServerProtocol, count: int):
    ...
    async def _broadcast_event(self, event: AnalysisEvent):
    ...
    def _should_send_to_client(self, client_id: str, event: AnalysisEvent) -> bool:
        ...
        if "subscriptions" in filters:
            if event.type not in filters["subscriptions"]:
                return False

        # Check priority filter
        if "priority" in filters:
            if event.priority.value > filters["priority"].value:
                return False

        # Check source filter
        if "source" in filters:
            if event.source != filters["source"]:
                return False

        # Check tag filters
        if "tags" in filters:
            if not any(tag in event.tags for tag in filters["tags"]):
                return False
    ...
    def publish_event(self, event: AnalysisEvent):
...
class EventPublisher:
    ...
    def __init__(self, stream: WebSocketEventStream, source: str):
